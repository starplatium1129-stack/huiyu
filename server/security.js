...
let envelope = require('./http-envelope');
...
function localOnly(req, res, next) {
    if (!isDirectLocalRequest(req))
        return envelope.fail(res, 403, '该操作仅限本机使用');
    next();
}
...
function safeLocalUrl(value) {
    ...
    if (url.protocol !== 'http:')
        return '';
    if (LOOPBACK_HOSTNAMES.indexOf(url.hostname.toLowerCase()) === -1)
        return '';
...
function hostGuard(config, getTunnelUrl) {
    return function (req, res, next) {
        let tunnelHost = '';
        try {
            tunnelHost = tunnelHostFromUrl(getTunnelUrl && getTunnelUrl());
        }
        catch (error) { }
        if (hostAllowed(req.headers.host, config.PORT, tunnelHost))
            return next();
...
function tokenAuth(token) {
    return function (req, res, next) {
        ...
        return res.status(403).send('<!DOCTYPE html><html><head><meta charset="utf-8"><title>绘遇 · HUIYU</title>' +
