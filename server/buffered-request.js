...
function requestBuffered(target, options) {
    return new Promise((resolve, reject) => {
        ...
        const makeError = options.makeError || ((kind, error) => error instanceof Error ? error : new Error(kind));
        let settled = false, deadline, chunks = [], size = 0;
        ...
        const request = (url.protocol === 'https:' ? https : http).request(url, {
            ...
            response.on('data', chunk => {
                ...
                size += chunk.length;
                if (size > limit)
                    return finish(makeError('tooLarge'));
            ...
            response.on('end', () => {
                ...
                if (!response.complete)
                    return finish(makeError('aborted'));
                finish(undefined, { status: response.statusCode || 0, headers: response.headers, body: Buffer.concat(chunks, size) });
            });
            if (request.method !== 'HEAD' && Number(response.headers['content-length']) > limit)
                finish(makeError('tooLarge'));
        });
...
module.exports = { requestBuffered };
