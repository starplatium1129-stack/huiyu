...
import type { RegisteredJob, TimerSlots } from './job-types';
...
function createJobRegistry<Job extends RegisteredJob = RegisteredJob>() {
  ...
  return {
     ...
     */
    armTimer: function <Key extends PropertyKey>(job: TimerSlots<Key>, slot: Key, delayMs: number|undefined, fn: () => void, options?: { unref?: boolean }) {
      ...
      job[slot] = timer;
      if (options && options.unref && typeof timer.unref === 'function') timer.unref();
      return timer;
    },
    clearTimer: function <Key extends PropertyKey>(job: TimerSlots<Key>, slot: Key) {
