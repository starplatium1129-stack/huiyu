...
import type { ComfyConfig, ComfyStateConfig, ComfyError, ComfyQueueResponse } from './comfy-types';
...
function stateDir(config?: ComfyStateConfig | null) {
...
function clientIdFor(config: ComfyStateConfig | null | undefined, namespace: string) {
...
async function requestComfy(config: ComfyConfig, method: string, pathname: string, body?: Record<string, unknown> | null, timeoutMs?: number, maxBytes?: number) {
  ...
  return requestBuffered(target, {
    ...
    makeError(kind: string, error) {
      ...
      if (kind === 'network' && (error as NodeJS.ErrnoException | undefined)?.code) return error!;
 ...
 */
async function unloadComfyModels(config: ComfyConfig) {
  try {
 ...
 */
async function cancelOrphanPrompts(config: ComfyConfig, clientId: string) {
  let cancelled = [];
  try {
    let data = await requestComfyJson<ComfyQueueResponse>(config, 'GET', '/queue', null, 8000) || {};
 ...
 */
function sweepOrphanPromptsAfterStart(config: ComfyConfig, clientId: string, logLabel: string) {
  let run = function () {
