...
export function isDesktopHost(): boolean {
  return '__TAURI_INTERNALS__' in window || '__TAURI__' in window
    || ['http://tauri.localhost', 'https://tauri.localhost', 'tauri://localhost'].includes(location.origin)
}
export async function initializePlatform(isBusy: () => boolean): Promise<() => void> {
  ...
  if (isDesktopHost()) {
    ...
    const sync = async () => {
      ...
      } else if (!selected && !['http://tauri.localhost', 'https://tauri.localhost', 'tauri://localhost'].includes(location.origin)) {
      ...
      } else if (!profileRuntimeActive() && !['http://tauri.localhost', 'https://tauri.localhost', 'tauri://localhost'].includes(location.origin)) setProfileConnectionBlocked(false)
    }
