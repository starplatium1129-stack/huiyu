...
for (const theme of ['dark', 'light']) {
  ...
  test(`reset profile-only content with keyboard ${theme}`, async ({ page, context }) => {
    ...
    const chatWindow = await context.newPage()
    await chatWindow.goto('/companion-chat')
    await chatWindow.locator('textarea').fill('neutral-window-draft')
    ...
    await expect(chatWindow.locator('textarea')).toHaveValue('')
    await chatWindow.reload()
    await expect(chatWindow.locator('textarea')).toHaveValue('')
