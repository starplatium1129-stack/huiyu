...
function validateInput(body: any, config?: any, options?: { isLocal: boolean; }|undefined) {
  ...
  let h3Lines = [
    ...
    dialogueLine,
    hasReferences
      ? 'Character identity anchors: ' + references.map(function (_: any, i: number) { return '<Picture ' + (i + 1) + '>'; }).join(', ')
        + (references.length === 1
