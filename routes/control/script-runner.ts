...
function createScriptRunner(options: { rootDir?: unknown; }) {
  options = options || {};
  ...
  function runScriptAsync(scriptPath: PathLike, args: unknown, timeoutMs: unknown) {
    return new Promise(function (resolve) {
      ...
      child.stdout.on('data', function (chunk: { toString: (arg0: string) => string; }) {
        if (stdout.length < OUTPUT_CAP) stdout += chunk.toString('utf8');
      });
      child.stderr.on('data', function (chunk: { toString: (arg0: string) => string; }) {
        if (stderr.length < OUTPUT_CAP) stderr += chunk.toString('utf8');
      });
      child.on('error', function (error: { message: unknown; }) { done({ ok:false, error:error.message }); });
