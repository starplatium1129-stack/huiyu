...
function validateInput(reqOrBody, expectedFamilyOrBody, maybeExpectedFamily) {
    ...
    if (!isPlainObject(body))
        throw serviceError(400, 'INVALID_BODY', '请求体必须是 JSON 对象');
    ...
    required.forEach(function (key) {
        if (!hasOwn(body, key))
            throw serviceError(400, 'MISSING_PARAMETER', '缺少参数：' + key);
    });
    ...
    if (!model)
        throw serviceError(400, 'UNKNOWN_MODEL', '未知生成模型');
    if (expectedFamily && model.family !== expectedFamily)
        throw serviceError(400, 'WRONG_ROUTE_FAMILY', '请求路径与模型 family 不匹配');
    var modelId = body.modelId;
    var expectedProfile = PROFILE_BY_MODEL[modelId];
    var lora = typeof body.loraId === 'string' && hasOwn(LORAS, body.loraId) ? LORAS[body.loraId] : null;
    if (model.family !== 'krea2' && body.styleLoraId !== undefined)
        throw serviceError(400, 'WRONG_ROUTE_FAMILY', 'Style LoRA 仅适用于 Krea 2');
    if (model.family === 'krea2') {
        if (body.loraId || body.loraStrength !== undefined || (body.negative && String(body.negative).trim()))
            throw serviceError(400, 'KREA_UNSUPPORTED_PARAMETER', 'Krea 2 不接受角色 LoRA 或负向 Prompt');
        if (body.styleLoraId !== undefined && (typeof body.styleLoraId !== 'string' || !hasOwn(KREA_STYLE_LORAS, body.styleLoraId)))
            throw serviceError(400, 'UNKNOWN_STYLE_LORA', '未知 Krea 2 官方 Style LoRA');
    }
        ...
        if (!lora)
            throw serviceError(400, 'UNKNOWN_LORA', '未知 Anima LoRA');
        if (lora.compatibleModels.indexOf(modelId) === -1)
    ...
    if (model.family !== 'krea2' && width * height > 1_850_000)
        throw serviceError(400, 'INVALID_PARAMETER', '输出尺寸超过允许面积');
    var steps;
    var cfg;
    if (model.family === 'krea2') {
        // 2026-08-31 与 KREA_DEFAULTS 对齐：e0cbf20 已把主 KSampler 实际出图步数改为 12
        if (body.steps !== undefined && body.steps !== 12)
            throw serviceError(400, 'INVALID_PARAMETER', 'Krea 2 steps 固定为 12');
        if (body.cfg !== undefined && body.cfg !== 1)
            throw serviceError(400, 'INVALID_PARAMETER', 'Krea 2 CFG 固定为 1');
        steps = 12;
        cfg = 1;
    }
    else {
        steps = body.steps === undefined ? model.steps : validateNumber(body.steps, 'steps', generationContract.PARAMETER_LIMITS.steps.min, generationContract.PARAMETER_LIMITS.steps.max, generationContract.PARAMETER_LIMITS.steps.integer);
        cfg = body.cfg === undefined ? model.cfg : validateNumber(body.cfg, 'cfg', generationContract.PARAMETER_LIMITS.cfg.min, generationContract.PARAMETER_LIMITS.cfg.max, generationContract.PARAMETER_LIMITS.cfg.integer);
    }
    var seed = body.seed === undefined
        ? crypto.randomInt(0, 2147483647)
        : validateNumber(body.seed, 'seed', generationContract.PARAMETER_LIMITS.seed.min, generationContract.PARAMETER_LIMITS.seed.max, generationContract.PARAMETER_LIMITS.seed.integer);
    return {
