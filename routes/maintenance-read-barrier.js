...
function maintenanceReadBarrier(options) {
  return function barrier(req, res, next) {
    if (!['GET', 'HEAD'].includes(req.method) || installed.has(res)) return next();
    let token;
    try { token = maintenanceReadToken(options); }
    catch (error) {
      res.set('Cache-Control', 'no-store');
      return res.status(error.statusCode || 503).json({ ok: false, code: error.code, error: error.message, recoveryRequired: true });
    }
    ...
    res.end = (chunk, encoding, callback) => {
      ...
      try {
        ...
        return original.end.call(res, JSON.stringify({ ok: false, code: error.code || 'MAINTENANCE_READ_BLOCKED', error: error.message, recoveryRequired: true }));
      }
...
module.exports = { maintenanceReadBarrier };
