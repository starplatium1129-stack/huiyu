...
const { parse, report, main } = require('../maintenance/report-content-impact');
const { collectGitHistory } = require('../maintenance/content-impact-git');
const { formatImpactReport } = require('../lib/content-impact-format');
const { fixture, git, snapshot } = require('./content-history-fixture');
const script = path.resolve(__dirname, '../maintenance/report-content-impact.js');
const run = (f, ...args) => report(parse(['--root', f.root, '--base', f.base, ...args]));
const entity = (result, kind, id, role = 'source') => result.history.entities.find((r) => r.kind === kind && r.id === id && r.role === role);
...
test('explicit base tracks committed changes, scoped IDs, both owners/outfits and separate source/derived records', (t) => {
  ...
  assert.ok(change.oldRelations.some((r) => r.kind === 'outfit' && r.characterId === 'a' && r.id === 'dress'));
  assert.ok(change.newRelations.some((r) => r.kind === 'outfit' && r.characterId === 'b' && r.id === 'coat'));
  assert.equal(entity(result, 'blueprint', 'bp-a', 'derived').change, 'modified');
  assert.ok(!result.affected.some((r) => r.id === 'bp-unrelated'));
  ...
  assert.ok(result.evidence.files.every((r) => r.sha256));
...
test('deleted source records survive in old relationships; unchanged stale aggregate is a mismatch, not a source', (t) => {
  ...
  rows.characters = rows.characters.filter((r) => r.id !== 'a');
  ...
  assert.ok(result.affected.some((r) => r.id === 'bp-a' && r.impact === 'revalidate'));
...
test('staged rename with changed source uses stable IDs across old/new manifest paths', (t) => {
  ...
  const result = run(f);
  assert.ok(result.gitHistory.changes.some((r) => r.status.startsWith('R') && r.oldPath === 'data/blueprints/one.json' && r.path === 'data/blueprints/renamed file.json'));
...
test('aggregate-only changes are identified by derived stable ID and preserve source authority', (t) => {
  ...
  assert.ok(result.affected.find((r) => r.id === 'bp-a').sources.some((r) => r.side === 'working-tree'));
});
...
test('scene deletion/creation, curation removal and retired additions include sc1000 and old members', (t) => {
  ...
  assert.ok(result.affected.some((r) => r.id === 'sc1000'));
  assert.ok(result.affected.some((r) => r.id === 'sc001'));
...
test('old/new default outfit changes invalidate implicit blueprint dependencies in both snapshots', (t) => {
  ...
  const result = run(f);
  const affected = result.affected.find((r) => r.id === 'bp-default');
  assert.ok(affected);
  assert.ok(affected.oldRelations.some((r) => r.kind === 'outfit' && r.id === 'dress' && r.via === 'proven-default-outfit'));
  assert.ok(affected.newRelations.some((r) => r.kind === 'outfit' && r.id === 'coat' && r.via === 'proven-default-outfit'));
  for (const id of ['dress', 'coat']) assert.ok(result.affected.some((r) => r.kind === 'outfit' && r.characterId === 'a' && r.id === id));
  assert.ok(!result.affected.some((r) => r.id === 'bp-unrelated'));
});
...
test('clean filters/external diff hooks cannot execute; Git commands have safe environment and argv', (t) => {
  ...
  t.mock.method(cp, 'spawnSync', (command, args, options) => {
    ...
    assert.ok(!args.some((arg) => ['fetch', 'pull', 'push', 'checkout', 'update-index'].includes(arg)));
    commands.push(args);
...
test('ordering-only and index-only changes identify stable IDs without inventing field edits', (t) => {
  ...
  assert.ok(result.affected.some((r) => r.kind === 'blueprint' && r.id === 'bp-a'));
  assert.equal(result.incrementalPlan.mode, 'full');
  ...
  assert.ok(indexResult.history.metadata.some((r) => r.file === 'data/scenes-index.json'));
  assert.ok(indexResult.affected.some((r) => r.kind === 'scene' && r.id === 'sc1000'));
...
test('partial/invalid Git NUL output is discarded and base blobs retain exact byte hashes', (t) => {
  ...
  assert.equal(read.sha256, require('node:crypto').createHash('sha256').update(fs.readFileSync(path.join(f.root, 'data/blueprints/one.json'))).digest('hex'));
  const spawn = cp.spawnSync;
  for (const stdout of [Buffer.from('M\0data/blueprints/one.json\0D\0missing-terminator'), Buffer.from('R100\0data/blueprints/one.json\0../escape\0'), Buffer.from([255, 0])]) {
    const mock = t.mock.method(cp, 'spawnSync', (command, args, options) => args.includes('diff')
