...
const {
  ...
  franchiseSlug,
  jsonText,
} = require('../lib/blueprint-change-plan');
...
function shardInputs(shards, textOverrides) {
  const out = {};
...
function plan(state, target, textOverrides) {
...
function planError(fn, substring) {
...
test('franchiseSlug 本地副本与 popular-store 原函数对拍，防止规则漂移', () => {
  const { franchiseSlug: storeSlug } = require('../lib/popular-store');
  for (const value of [
...
test('修改单条蓝图：只写对应分片，组内顺序与字段保留，manifest 计数不变', () => {
  ...
  assert.deepEqual(shard.blueprints.map((b) => b.id), ['frieren_1', 'frieren_2']);
...
test('新增系列：slug 命名、manifest 追加、聚合按 manifest 顺序拼接', () => {
  ...
  assert.deepEqual(p.manifest.data.files.map((f) => f.file), [FRIEREN_FILE, FATE_FILE, MARIN_FILE]);
...
test('多个新系列按目标首次出现顺序追加；既有系列顺序不受目标顺序影响', () => {
  ...
  const p = plan(state, target);
  assert.deepEqual(p.manifest.data.files.map((f) => f.file),
    [FRIEREN_FILE, FATE_FILE, 'hololive.json', MARIN_FILE]);

  const reversed = [...state.target].reverse();
  const p2 = plan(state, reversed);
  assert.deepEqual(p2.manifest.data.files.map((f) => f.file), [FRIEREN_FILE, FATE_FILE]);
  ...
  assert.deepEqual(JSON.parse(p2.writes[0].text).blueprints.map((b) => b.id), ['frieren_2', 'frieren_1']);
});
...
test('删除组内一条是重写；删除最后一条是删片并移除 manifest 项', () => {
  ...
  assert.deepEqual(partial.manifest.data.files.map((f) => [f.file, f.count]),
  ...
  assert.deepEqual(emptied.manifest.data.files.map((f) => f.franchise), [FRIEREN]);
...
test('跨系列移动：旧片移除、新片加入；旧系列清空时删片', () => {
  ...
  assert.deepEqual(JSON.parse(p.writes[0].text).blueprints.map((b) => b.id), ['frieren_1']);
  assert.deepEqual(JSON.parse(p.writes[1].text).blueprints.map((b) => b.id), ['frieren_2', 'fate_1']);
  assert.deepEqual(p.manifest.data.files.map((f) => [f.file, f.count]),
  ...
  assert.deepEqual(p2.writes.map((w) => w.file), [FRIEREN_FILE]);
  assert.deepEqual(JSON.parse(p2.writes[0].text).blueprints.map((b) => b.id),
...
test('新系列冲突：与既有分片同名（大小写不敏感）、新系列互撞、unknown slug、保留设备名', () => {
  const state = baseState();
  const planWith = (blueprints, extraMapping) => planBlueprintChanges({
    manifest: state.manifest,
...
test('未知角色与无法确认的 franchise 明确报错，不自动建 unknown 片', () => {
  const state = baseState();
  const planWithMapping = (mapping) => planBlueprintChanges({
    manifest: state.manifest,
    shards: shardInputs(state.shards),
...
test('来源分片不齐全、多余分片、franchise 冲突、text/data 不一致、来源 id 问题均拒绝', () => {
  ...
  const planWithShards = (shards) => planBlueprintChanges({
    manifest: state.manifest,
    shards,
...
test('落盘夹具上真实 blueprint-store 对账：聚合字节一致，重规划收敛为零写', (t) => {
  ...
  const store = require('../lib/blueprint-store');
  ...
  assert.deepEqual(loaded.blueprints.map((b) => b.id), target.map((b) => b.id));
  store.writeBlueprintAggregate();
  ...
  assert.deepEqual(p2.unchanged.map((u) => u.file), manifest2.files.map((f) => f.file));
});
