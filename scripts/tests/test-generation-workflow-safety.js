...
const F = require('./generation-safety-fixture');
const { gatewayUrl, validatePng } = require('../lib/generation-gateway');
const { buildReview } = require('../maintenance/build-scene-manual-review');
const { planPublished, assertFullReviewCoverage } = require('../maintenance/publish-scene-showcase-anima11');
const scenePublisher = require('../maintenance/publish-scene-showcase-anima11');
const popularPublisher = require('../maintenance/publish-popular-showcase');
...
test('timeout and concurrent ownership cannot silently resubmit a running job', async t => {
  ...
  assert.equal(records(f)[0].status, 'recoverable'); assert.equal(mock.state.posts.length, 1);
  F.writeJson(path.join(f.output, '.generation.lock'), { pid: process.pid, host: require('node:os').hostname() });
