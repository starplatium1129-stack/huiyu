...
const { acquireMaintenanceLease } = require('../lib/maintenance-lease');
const { prepareMaintenanceTransaction } = require('../lib/maintenance-transaction');
const { readBackup, restoreEntries } = require('../lib/maintenance-recovery-backup');
const { applyMaintenanceRecovery } = require('../lib/maintenance-recovery');
const { names } = require('./maintenance-recovery-fixture');
...
catch (error) { process.send({ status: 'blocked', code: error.code }, () => process.exit(0)); }
if (lease) {
  let backup;
  let participant;
  ...
  if (mode !== 'preparing') {
    ...
    for (const file of options.additionalFiles || []) io.atomicWrite(file, 'partial-external-showcase', true);
      ...
      if (mode !== 'stream-race' || process.platform !== 'win32' || !['EPERM', 'EBUSY'].includes(error.code)
        || !error.dest || !options.readOpenTarget || !io.samePath(error.dest, options.readOpenTarget)) throw error;
      writeBlocked = { code: error.code, target: error.dest };
    }
