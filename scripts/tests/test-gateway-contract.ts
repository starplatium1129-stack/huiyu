...
test('SD 代理：超过 15 秒的生成正常返回，复用连接不累积超时监听器', { timeout:25000 }, async () => {
  ...
  stack.server.on('request', (req: any, res: any) => {
  ...
  try {
    for (let attempt = 0; attempt < 15; attempt += 1) {
      assert.equal((await request('/sdapi/v1/progress') as any).status, 200);
    }
    ...
    const generated: any = await request('/sdapi/v1/txt2img', 'POST');
...
test('gateway WebSocket：仅本机可升级，持令牌的远程连接也不能获得原生双向通道', async () => {
  ...
  stack.upstreams.sd.mock.server.on('upgrade', (req: any, socket: any) => {
    upstreamUpgrades += 1;
...
let PORT = 0;
let TOKEN = 'contract-token-0123456789abcdef0123456789ab';
let LOCAL: any = null;
// 隧道请求的形状：socket 来自 127.0.0.1（cloudflared），但带转发头。
let TUNNELED: any = null;
...
async function main() {
  let stack = await gatewayTestStack.start({ token:TOKEN });
  let desktopStack = null;
  ...
  try {
    // ---- S-1: 隧道请求不得被当成本机 ----
    let tunneledConfig: any = await postJson('/api/config', { sdHost:'http://127.0.0.1:1234' }, TUNNELED);
    assert.strictEqual(tunneledConfig.status, 403,
      'POST /api/config over tunnel must be 403, got ' + tunneledConfig.status);

    let localOnlyRoutes = ['/api/status', '/api/logs', '/api/diagnostics', '/api/share-link'];
    for (let i = 0; i < localOnlyRoutes.length; i++) {
      let tunneled: any = await request({ path:localOnlyRoutes[i], headers:TUNNELED });
      assert.strictEqual(tunneled.status, 403,
        localOnlyRoutes[i] + ' over tunnel must be 403, got ' + tunneled.status);
    }

    let localStatus: any = await request({ path:'/api/status', headers:LOCAL });
    ...
    let beforeCrossSite = fs.existsSync(stack.runtime.config) ? fs.readFileSync(stack.runtime.config, 'utf8') : null;
    let crossSiteStart: any = await request({
    ...
    let externalTool: any = await postJson('/api/desktop-tools', { name:'get_workspace_info', args:{} },
    ...
    let developmentStart: any = await postJson('/api/start', { enableTunnel:false },
    ...
    let firstVisit: any = await request({
    ...
    let ssrfHosts = ['http://169.254.169.254', 'http://evil.example.com:7860', 'https://127.0.0.1:7860', 'http://127.0.0.1:0'];
    for (let h = 0; h < ssrfHosts.length; h++) {
      let rejected: any = await postJson('/api/config', { sdHost:ssrfHosts[h] });
      assert.strictEqual(rejected.status, 400, ssrfHosts[h] + ' must be rejected with 400');
    }
    let accepted: any = await postJson('/api/config', { sdHost:stack.config.SD_HOST });
    assert.strictEqual(accepted.status, 200, 'loopback sdHost must be accepted');
    let comfyAccepted: any = await postJson('/api/config', { comfyHost:'http://127.0.0.1:8189' });
    ...
    let comfyRejected: any = await postJson('/api/config', { comfyHost:'http://169.254.169.254:8188' });
    assert.strictEqual(comfyRejected.status, 400, 'non-loopback comfyHost must be rejected');

    // ---- S-4: 原始 token 不得出现在轮询接口里；Host 白名单生效 ----
    let status: any = await request({ path:'/api/status', headers:LOCAL });
    ...
    let foreignHost: any = await request({ path:'/api/health', headers:{ Host:'attacker.example.com' } });
    assert.strictEqual(foreignHost.status, 421, 'foreign Host must be refused (DNS rebinding guard)');

    let diagnostics: any = await request({ path:'/api/diagnostics', headers:LOCAL });
    ...
    let unauthUpgrade: any = await upgradeRequest({ Host:'127.0.0.1:' + PORT, 'x-forwarded-for':'9.9.9.9' });
    ...
    let aliveAfterUpgrade: any = await request({ path:'/api/health', headers:LOCAL });
    ...
    // ---- B-3: 状态码与错误信封 ----
    let missingImage: any = await request({ path:'/scene-showcase/images/sc999.jpg', headers:LOCAL });
    ...
    let oversize: any = await postJson('/api/translate', { text:'x'.repeat(70000) });
    assert.strictEqual(oversize.status, 413, 'oversize body must be 413, not 500');

    let badJson: any = await request({
      method:'POST',
      path:'/api/translate',
    ...
    let unknownApi: any = await request({ path:'/api/does-not-exist', headers:LOCAL });
    ...
    // 前端路由仍应回 SPA 外壳
    let spaRoute: any = await request({ path:'/scene-explorer', headers:LOCAL });
    ...
    // 装了扩展还能碰文件系统。
    let sdBlocked = ['/sdapi/v1/options-anything', '/controlnet/model_list', '/adetailer/v1/version'];
    for (let b = 0; b < sdBlocked.length; b++) {
      let denied: any = await request({ path:sdBlocked[b], headers:LOCAL });
    ...
    for (let cb = 0; cb < comfyBlocked.length; cb++) {
      let comfyDenied: any = await request({ path:comfyBlocked[cb], headers:LOCAL });
    ...
    let remoteAnima: any = await request({
    ...
    for (let e = 0; e < errorCases.length; e++) {
      let envelopeCase: any = errorCases[e];
      let body = envelopeCase.res.json;
    ...
    // 成功信封同样固定：ok:true
    let okConfig: any = await postJson('/api/config', { sdHost:stack.config.SD_HOST });
    ...
    let statusSiblings = ['/api/status', '/api/sd-status', '/api/tts-status', '/api/chat-status'];
    for (let s = 0; s < statusSiblings.length; s++) {
      let probe: any = await request({ path:statusSiblings[s], headers:LOCAL });
    ...
    let burst = await Promise.all(Array.from({ length:24 }, function () {
      return postJson('/sdapi/v1/txt2img', { prompt:'probe' }, TUNNELED);
    }));
    assert.ok(burst.every(function (shot: any) { return shot.status === 403 && shot.json.code === 'SD_NATIVE_LOCAL_ONLY'; }),
      'every remote native write must be denied, including the first request');

    // 本机直连是电脑主人，不该被自己的限流挡住。
    // SD 未启动时代理回 502，这里只断言"不是 429"。
    for (let lg = 0; lg < 8; lg++) {
      let localShot: any = await postJson('/sdapi/v1/txt2img', { prompt:'probe' }, LOCAL);
      assert.notStrictEqual(localShot.status, 429,
        'direct-local requests must never be rate limited (attempt ' + (lg + 1) + ')');
    }

    // 廉价读端点不该共用出图的桶
    let cheapRead: any = await request({ path:'/sdapi/v1/samplers', headers:TUNNELED });
    ...
    for (let d = 0; d < privateData.length; d++) {
      let hidden: any = await request({ path:'/data/' + privateData[d], headers:LOCAL });
    ...
    if (fs.existsSync(distApp)) {
      let hashed = fs.readdirSync(distApp).filter(function (f) { return /-[\w-]{8,}\.js$/.test(f); })[0];
      if (hashed) {
        let assetRes: any = await request({ path:'/_app/' + hashed, headers:LOCAL });
        let cc = String(assetRes.headers['cache-control'] || '');
        assert.ok(cc.indexOf('immutable') !== -1 && cc.indexOf('max-age=31536000') !== -1,
          'content-hashed assets must be immutable, got ' + cc);
      }
      let shellRes: any = await request({ path:'/', headers:LOCAL });
    ...
    let designSystem: any = await request({ path:'/src/assets/css/design-system.css', headers:LOCAL });
    assert.strictEqual(designSystem.status, 200, 'docs pages need the canonical design system');
    let srcLeak: any = await request({ path:'/src/main.ts', headers:LOCAL });
    ...
    if (fs.existsSync(path.join(__dirname, '..', '..', 'data', 'scenes.json.br'))) {
      let brotli: any = await request({ path:'/data/scenes.json', headers:Object.assign({ 'Accept-Encoding':'br' }, LOCAL) });
      ...
      // 预压查找不得成为目录穿越入口
      let traversal: any = await request({ path:'/data/../server.js', headers:Object.assign({ 'Accept-Encoding':'br' }, LOCAL) });
    ...
    let DESKTOP_PORT = desktopStack.address.port;
    let desktopLocal = { Host:'127.0.0.1:' + DESKTOP_PORT };
    let scenesPost: any = await request({
    ...
    let buildWeb: any = await request({ port:DESKTOP_PORT, method:'POST', path:'/api/maintenance/build-web', headers:desktopLocal });
