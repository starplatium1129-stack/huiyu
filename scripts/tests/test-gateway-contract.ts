...
async function main() {
  ...
  try {
    // ---- S-1: 隧道请求不得被当成本机 ----
    var tunneledConfig: any = await postJson('/api/config', { sdHost:'http://127.0.0.1:1234' }, TUNNELED);
    ...
    for (var i = 0; i < localOnlyRoutes.length; i++) {
      var tunneled: any = await request({ path:localOnlyRoutes[i], headers:TUNNELED });
      assert.strictEqual(tunneled.status, 403,
        localOnlyRoutes[i] + ' over tunnel must be 403, got ' + tunneled.status);
    }

    var localStatus: any = await request({ path:'/api/status', headers:LOCAL });
    ...
    var crossSiteStart: any = await request({
    ...
    var externalTool: any = await postJson('/api/desktop-tools', { name:'get_workspace_info', args:{} },
    ...
    var developmentStart: any = await postJson('/api/start', { enableTunnel:false },
    ...
    var firstVisit: any = await request({
    ...
    for (var h = 0; h < ssrfHosts.length; h++) {
      var rejected: any = await postJson('/api/config', { sdHost:ssrfHosts[h] });
      assert.strictEqual(rejected.status, 400, ssrfHosts[h] + ' must be rejected with 400');
    }
    var accepted: any = await postJson('/api/config', { sdHost:stack.config.SD_HOST });
    assert.strictEqual(accepted.status, 200, 'loopback sdHost must be accepted');
    var comfyAccepted: any = await postJson('/api/config', { comfyHost:'http://127.0.0.1:8189' });
    ...
    var comfyRejected: any = await postJson('/api/config', { comfyHost:'http://169.254.169.254:8188' });
    assert.strictEqual(comfyRejected.status, 400, 'non-loopback comfyHost must be rejected');

    // ---- S-4: 原始 token 不得出现在轮询接口里；Host 白名单生效 ----
    var status: any = await request({ path:'/api/status', headers:LOCAL });
    ...
    var foreignHost: any = await request({ path:'/api/health', headers:{ Host:'attacker.example.com' } });
    assert.strictEqual(foreignHost.status, 421, 'foreign Host must be refused (DNS rebinding guard)');

    var diagnostics: any = await request({ path:'/api/diagnostics', headers:LOCAL });
    ...
    var unauthUpgrade: any = await upgradeRequest({ Host:'127.0.0.1:' + PORT, 'x-forwarded-for':'9.9.9.9' });
    ...
    var aliveAfterUpgrade: any = await request({ path:'/api/health', headers:LOCAL });
    ...
    // ---- B-3: 状态码与错误信封 ----
    var missingImage: any = await request({ path:'/scene-showcase/images/sc999.jpg', headers:LOCAL });
    ...
    var oversize: any = await postJson('/api/translate', { text:'x'.repeat(70000) });
    assert.strictEqual(oversize.status, 413, 'oversize body must be 413, not 500');

    var badJson: any = await request({
      method:'POST',
      path:'/api/translate',
    ...
    var unknownApi: any = await request({ path:'/api/does-not-exist', headers:LOCAL });
    ...
    // 前端路由仍应回 SPA 外壳
    var spaRoute: any = await request({ path:'/scene-explorer', headers:LOCAL });
    ...
    for (var b = 0; b < sdBlocked.length; b++) {
      var denied: any = await request({ path:sdBlocked[b], headers:LOCAL });
    ...
    for (var cb = 0; cb < comfyBlocked.length; cb++) {
      var comfyDenied: any = await request({ path:comfyBlocked[cb], headers:LOCAL });
    ...
    var remoteAnima: any = await request({
    ...
    for (var e = 0; e < errorCases.length; e++) {
      var envelopeCase: any = errorCases[e];
    ...
    // 成功信封同样固定：ok:true
    var okConfig: any = await postJson('/api/config', { sdHost:stack.config.SD_HOST });
    ...
    for (var s = 0; s < statusSiblings.length; s++) {
      var probe: any = await request({ path:statusSiblings[s], headers:LOCAL });
    ...
    for (var lg = 0; lg < 8; lg++) {
      var localShot: any = await postJson('/sdapi/v1/txt2img', { prompt:'probe' }, LOCAL);
      assert.notStrictEqual(localShot.status, 429,
        'direct-local requests must never be rate limited (attempt ' + (lg + 1) + ')');
    }

    // 廉价读端点不该共用出图的桶
    var cheapRead: any = await request({ path:'/sdapi/v1/samplers', headers:TUNNELED });
    ...
    for (var d = 0; d < privateData.length; d++) {
      var hidden: any = await request({ path:'/data/' + privateData[d], headers:LOCAL });
    ...
    for (var pd = 0; pd < publicData.length; pd++) {
      var served: any = await request({ path:'/data/' + publicData[pd], headers:LOCAL });
    ...
    if (fs.existsSync(distApp)) {
      ...
      if (hashed) {
        var assetRes: any = await request({ path:'/_app/' + hashed, headers:LOCAL });
      ...
      var shellRes: any = await request({ path:'/', headers:LOCAL });
    ...
    var designSystem: any = await request({ path:'/src/assets/css/design-system.css', headers:LOCAL });
    assert.strictEqual(designSystem.status, 200, 'docs pages need the canonical design system');
    var srcLeak: any = await request({ path:'/src/main.ts', headers:LOCAL });
    ...
    if (fs.existsSync(path.join(__dirname, '..', '..', 'data', 'scenes.json.br'))) {
      var brotli: any = await request({ path:'/data/scenes.json', headers:Object.assign({ 'Accept-Encoding':'br' }, LOCAL) });
      ...
      var gzipped: any = await request({ path:'/data/scenes.json', headers:Object.assign({ 'Accept-Encoding':'gzip' }, LOCAL) });
      ...
      var identity: any = await request({ path:'/data/scenes.json', headers:Object.assign({ 'Accept-Encoding':'identity' }, LOCAL) });
      ...
      // 预压查找不得成为目录穿越入口
      var traversal: any = await request({ path:'/data/../server.js', headers:Object.assign({ 'Accept-Encoding':'br' }, LOCAL) });
    ...
    var scenesPost: any = await request({
    ...
    var buildWeb: any = await request({ port:DESKTOP_PORT, method:'POST', path:'/api/maintenance/build-web', headers:desktopLocal });
