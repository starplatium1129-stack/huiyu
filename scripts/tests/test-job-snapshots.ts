...
function sampleJob(overrides: Record<string, unknown> = {}) {
  return Object.assign({ id:'v-1730000000-ab12', owner:'local', createdAt:1730000000000,
    estimatedSeconds:180, input:{ modelId:'h3-native', width:832, height:480, duration:5, family:'video' } }, overrides);
}
...
test('save writes only public fields, without prompts or tokens', t => {
  ...
  assert.deepEqual(Object.keys(parsed.input), ['modelId', 'width', 'height', 'duration', 'family']);
  assert.equal(parsed.input.family, 'video');
