...
test('inpaint canvas helper protects from oversize stretching and 16-aligned', function () {
  let source = fs.readFileSync(path.join(__dirname, '..', '..', 'src', 'utils', 'inpaintCanvas.ts'), 'utf8');
...
test('AnimaInpaintModal delegates sizing to the shared canvas helper', function () {
  ...
  let source = fs.readFileSync(path.join(__dirname, '..', '..', 'src', 'components', 'inpaint', 'useInpaintImageSource.ts'), 'utf8');
