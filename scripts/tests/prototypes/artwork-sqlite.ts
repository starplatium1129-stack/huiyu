...
function openArtworkCandidate(root, fault = () => {}) {
  ...
  function importSnapshot(id, snapshot) {
    const manifest = snapshot.images.map(image => ({ id: image.id, hash: digest(image.bytes), bytes: image.bytes.length }));
    if (new Set(manifest.map(image => image.id)).size !== manifest.length) throw new Error('Duplicate image ID');
    if (new Set(snapshot.history.map(entry => entry.id)).size !== snapshot.history.length) throw new Error('Duplicate artwork ID');
    const images = new Set(manifest.map(image => image.id));
    const works = new Set(snapshot.history.map(entry => entry.id));
    for (const entry of [...snapshot.history, ...snapshot.trash.flatMap(entry => entry.historyEntries || [])]) {
      if (entry.image_id && !images.has(entry.image_id)) throw new Error('Missing referenced original');
    }
    for (const entry of snapshot.trash) {
      if ((entry.imageIds || []).some(id => !images.has(id))) throw new Error('Missing trash original');
    }
    for (const project of snapshot.projects) {
      if ((project.history_ids || []).some(id => !works.has(id))) throw new Error('Missing project artwork');
    }
  ...
  function publishMetadata(history, projects, trash) {
    transaction(() => {
  ...
  return {
    ...
    state: id => db.prepare('SELECT state FROM migrations WHERE id=?').get(id)?.state,
