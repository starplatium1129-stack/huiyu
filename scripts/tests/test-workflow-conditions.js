...
const { reportConditions, formatConditions, formatConditionRow, main } = require('../maintenance/report-workflow-conditions');
function fixture(t) {
...
test('conditions: JSON adds declared switches, notes and needs with stable defaults', t => {
  ...
  const a = result.commands.find(row => row.name === 'test:a');
  ...
  const b = result.commands.find(row => row.name === 'test:b');
...
test('conditions: composite rows keep their own metadata, not children merged', t => {
  ...
  const row = reportConditions({ root, registry }).commands.find(item => item.name === 'test:full');
...
test('conditions: --domain filtering and unknown domain rejection unchanged', t => {
  ...
  const filtered = reportConditions({ root, registry, domain: 'test' });
