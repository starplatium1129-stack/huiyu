...
async function waitForJob(base: string, id: string|number|boolean) {
  ...
  while (Date.now() < deadline) {
    let result = await json(await fetch(base + '/api/video/jobs/' + encodeURIComponent(id)));
...
async function run() {
  ...
  let graph = video.buildWorkflow(input);
  ...
  let h3Input = video.validateInput(validBody({ modelId:'minimax-h3' }));
  ...
  let h3Conflict = video.validateInput(validBody({
    modelId:'minimax-h3',
  ...
  let h3EnConflict = video.validateInput(validBody({
    modelId:'minimax-h3',
  ...
  let h3Rain = video.validateInput(validBody({
    modelId:'minimax-h3',
  ...
  let h3Graph = video.buildWorkflow(h3Input);
  ...
  let h3I2vInput = video.validateInput(validBody({
    modelId:'minimax-h3',
  ...
  let h3I2vGraph = video.buildWorkflow(h3I2vInput);
  ...
  let fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'aics-video-original-'));
  try {
    let fixtureConfig = { AI_WORKSPACE_ROOT: fixtureRoot, ROOT_DIR: path.resolve(__dirname, '..', '..') };
    let fixtureInputDir = path.join(fixtureRoot, 'ComfyUI', 'input');
    fs.mkdirSync(fixtureInputDir, { recursive:true });
    let tallPng = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==', 'base64');
    tallPng.writeUInt32BE(832, 16);
    tallPng.writeUInt32BE(1216, 20);
    let fixtureName = 'aics_video_input_' + 'a'.repeat(16) + '.png';
    fs.writeFileSync(path.join(fixtureInputDir, fixtureName), tallPng);

    let originalInput = video.validateInput(validBody({
    ...
    let originalGraph = video.buildWorkflow(originalInput);
  ...
  let h3Dialogue = video.validateInput(validBody({
    modelId:'minimax-h3',
    dialogue:'我在这站下车。',
  }));
  ...
  let h3EnDialogue = video.validateInput(validBody({
    modelId:'minimax-h3',
  ...
  let h3JpDialogue = video.validateInput(validBody({
    modelId:'minimax-h3',
    dialogue:'ありがとう。本当に。',
  }));
  ...
  let h3ExplicitZh = video.validateInput(validBody({
    modelId:'minimax-h3',
    dialogue:'ありがとう。本当に。',
    dialogueLang:'zh',
  }));
  ...
  let h3ExplicitJa = video.validateInput(validBody({
    modelId:'minimax-h3',
    dialogue:'我在这站下车。',
    dialogueLang:'ja',
  }));
  ...
  let h3ExplicitEn = video.validateInput(validBody({
    modelId:'minimax-h3',
    dialogue:'我在这站下车。',
    dialogueLang:'en',
  }));
  ...
  // ── P5 景别（官方 4.1 构图句）──────────────────────────────────────────
  let h3Wide = video.validateInput(validBody({ modelId:'minimax-h3', shotSize:'wide' }));
  ...
  let h3Fast4 = video.validateInput(validBody({ modelId:'minimax-h3', steps:4 }));
  ...
  let fl2vaInput = video.validateInput(validBody({
    modelId:'minimax-h3',
    image:'aics_video_input_abcdef0123456789.png',
  ...
  let fl2vaGraph = video.buildWorkflow(fl2vaInput);
  ...
  let l2vaInput = video.validateInput(validBody({
    modelId:'minimax-h3',
    lastFrame:'aics_video_input_0123456789abcdef.png',
    duration:3,
  }));
  ...
  let l2vaGraph = video.buildWorkflow(l2vaInput);
  ...
  let adultPrompt = validBody({ prompt:'nude explicit scene, naked character', adultEnabled:true });
  ...
  let batchAdult = video.validateBatchInput({
    modelId:'minimax-h3',
    aspectRatio:'landscape',
    adultEnabled:true,
  ...
  try {
    let t8Input = video.validateInput(validBody({ modelId:'minimax-h3', steps:4 }));
    let t8Graph = video.buildWorkflow(t8Input);
    ...
    let t8I2v = video.buildWorkflow(video.validateInput(validBody({
    ...
    let t8Fl2v = video.buildWorkflow(video.validateInput(validBody({
      modelId:'minimax-h3', image:'aics_video_input_abcdef0123456789.png',
      lastFrame:'aics_video_input_0123456789abcdef.png',
    })));
    assert.equal(t8Fl2v['5'].inputs.task_type, 'FL2VA');
    assert.deepEqual(t8Fl2v['5'].inputs.last_frame, ['18', 0]);
    let t8L2v = video.buildWorkflow(video.validateInput(validBody({
    ...
    let t8Ref = video.buildWorkflow(video.validateInput(validBody({
      modelId:'minimax-h3',
    ...
    let t8Hybrid = video.buildWorkflow(video.validateInput(validBody({
      modelId:'minimax-h3',
      image:'aics_video_input_abcdef0123456789.png',
  ...
  let storyboardEngine: typeof import('../../routes/video/storyboard') = require('../../routes/video/storyboard');
  let fixtureBlueprint = {
  ...
  let fixtureBoard = storyboardEngine.buildStoryboard(fixtureBlueprint, { intent:'她把一朵花别到耳边' });
  ...
  let noProseBoard = storyboardEngine.buildStoryboard(
  ...
  let storyboardShotInput = video.validateInput({
  ...
  let realBlueprints = JSON.parse(fs.readFileSync(
    path.join(__dirname, '..', '..', 'data', 'scene-blueprints.json'), 'utf8')).blueprints;
  let normalBlueprint = realBlueprints.find(function (item: any) { return item.category === '现代日常' && /「/.test(item.description || ''); });
  let adultBlueprint = realBlueprints.find(function (item: { category: string; }) { return item.category === '成人' || item.category === '私密写真'; });
  assert.ok(normalBlueprint && adultBlueprint, 'fixture data must contain normal and adult blueprints');

  let missingStack = await gatewayStack.start();
  try {
    let boardResponse = await fetch(missingStack.baseUrl + '/api/video/storyboard', {
      method:'POST', headers:{ 'content-type':'application/json' },
      body:JSON.stringify({ blueprintId:normalBlueprint.id }),
    });
    assert.equal(boardResponse.status, 200);
    let boardBody = await json(boardResponse);
    ...
    let quoted = (normalBlueprint.description.match(/「[^「」]+」/g) || []).map(function (raw: string|any[]) { return raw.slice(1, -1); });
    if (quoted.length) {
      assert.equal(boardBody.storyboard.shots[1].dialogue, quoted[0], 'server extraction matches the fixture expectation');
    }
    let unknownBoard = await fetch(missingStack.baseUrl + '/api/video/storyboard', {
      method:'POST', headers:{ 'content-type':'application/json' },
    ...
    let adultBoard = await fetch(missingStack.baseUrl + '/api/video/storyboard', {
      method:'POST', headers:{ 'content-type':'application/json' },
    ...
    let status = await json(await fetch(missingStack.baseUrl + '/api/video/status'));
    ...
    let wan = status.models.find(function (model: { id: string; }) { return model.id === 'wan2.2-ti2v-5b'; });
    ...
    let h3 = status.models.find(function (model: { id: string; }) { return model.id === 'minimax-h3'; });
    ...
    let missingResponse = await post(missingStack.baseUrl, validBody());
    assert.equal(missingResponse.status, 503);
    assert.equal((await json(missingResponse)).code, 'VIDEO_MODEL_UNAVAILABLE');
    let h3MissingResponse = await post(missingStack.baseUrl, validBody({ modelId:'minimax-h3' }));
    assert.equal(h3MissingResponse.status, 503);
    assert.equal((await json(h3MissingResponse)).code, 'VIDEO_MODEL_UNAVAILABLE');
    let unknownResponse = await post(missingStack.baseUrl, validBody({ workflow:{} }));
  ...
  let readyStack = await gatewayStack.start({
    prepare:function (context: { config: { AI_WORKSPACE_ROOT: string; }; }) {
      ...
      for (let model of video.constants.MODEL_CATALOG) {
        for (let requirement of model.requirements) {
          let dir = path.join(root, requirement[0]);
  ...
  try {
    ...
    let comfyState = await json(await fetch(readyStack.upstreams.comfy.url + '/__mock/state'));
    let promptCall = comfyState.calls.find(function (call: { path: string; }) { return call.path === '/prompt'; });
    ...
    let finished = await waitForJob(readyStack.baseUrl, created.id);
    assert.equal(finished.status, 'succeeded');
    assert.equal(finished.resultAvailable, true);
    let result = await fetch(readyStack.baseUrl + finished.resultUrl, {
    ...
    let h3CreateResponse = await post(readyStack.baseUrl, validBody({ modelId:'minimax-h3' }));
    assert.equal(h3CreateResponse.status, 202);
    let h3Created = (await json(h3CreateResponse)).job;
    assert.equal(h3Created.modelId, 'minimax-h3');
    assert.equal(h3Created.width, 832);
    assert.equal(h3Created.height, 480);

    let h3State = await json(await fetch(readyStack.upstreams.comfy.url + '/__mock/state'));
    let h3PromptCall = h3State.calls.find(function (call: { path: string; body: { prompt: { [x: string]: { class_type: string; }; }; }; }) {
    ...
    let h3Finished = await waitForJob(readyStack.baseUrl, h3Created.id);
    ...
    let badUpload = await fetch(readyStack.baseUrl + '/api/video/images', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
    ...
    let emptyUpload = await fetch(readyStack.baseUrl + '/api/video/images', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
      body:JSON.stringify({}),
    });
    ...
    let h3I2vJob = (await json(h3I2vCreate)).job;
    let i2vState = await json(await fetch(readyStack.upstreams.comfy.url + '/__mock/state'));
    let i2vCall = i2vState.calls.find(function (call: { path: string; body: { prompt: { [x: string]: { class_type: string; }; }; }; }) {
    ...
    let i2vFinished = await waitForJob(readyStack.baseUrl, h3I2vJob.id);
    ...
    let tallName = (await json(tallUpload)).name;
    let originalCreate = await post(readyStack.baseUrl, validBody({
      modelId:'minimax-h3',
      image:tallName,
    ...
    let originalJob = (await json(originalCreate)).job;
    assert.equal(originalJob.width, 512, 'job reports the ratio-fitted canvas');
    assert.equal(originalJob.height, 768);
    let originalState = await json(await fetch(readyStack.upstreams.comfy.url + '/__mock/state'));
    let originalCall = originalState.calls.find(function (call: { path: string; body: { prompt: { [x: string]: { inputs: { width: number; }; }; }; }; }) {
    ...
    let originalFinished = await waitForJob(readyStack.baseUrl, originalJob.id);
  ...
  let t8Stack = await gatewayStack.start({
    prepare:function (context: { config: { AI_WORKSPACE_ROOT: string; }; }) {
      let root = path.join(context.config.AI_WORKSPACE_ROOT, 'ComfyUI', 'models');
      for (let model of video.constants.MODEL_CATALOG) {
        for (let requirement of model.requirements) {
  ...
  try {
    let t8Create = await post(t8Stack.baseUrl, validBody({ modelId:'minimax-h3', steps:4 }));
    assert.equal(t8Create.status, 202);
    let t8Job = (await json(t8Create)).job;
    let t8State = await json(await fetch(t8Stack.upstreams.comfy.url + '/__mock/state'));
    let t8Call = t8State.calls.find(function (call: { path: string; body: { prompt: { [x: string]: { class_type: string; }; }; }; }) {
    ...
    let t8Finished = await waitForJob(t8Stack.baseUrl, t8Job.id);
  ...
  let batchStack = await gatewayStack.start({
    prepare:function (context: { config: { AI_WORKSPACE_ROOT: string; }; }) {
      let root = path.join(context.config.AI_WORKSPACE_ROOT, 'ComfyUI', 'models');
      for (let model of video.constants.MODEL_CATALOG) {
        for (let requirement of model.requirements) {
  ...
  try {
    ...
    let emptyShots = await fetch(batchStack.baseUrl + '/api/video/batches', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
      body:JSON.stringify({ modelId:'minimax-h3', aspectRatio:'landscape', shots:[] }),
    });
    assert.equal(emptyShots.status, 400);
    let badShotKey = await fetch(batchStack.baseUrl + '/api/video/batches', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
    ...
    let tooMany = await fetch(batchStack.baseUrl + '/api/video/batches', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
      body:JSON.stringify({
    ...
    let shot1Upload = await fetch(batchStack.baseUrl + '/api/video/images', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
      body:JSON.stringify({ data:tinyPngBase64 }),
    });
    let shot1Name = (await json(shot1Upload)).name;
    let shot3Upload = await fetch(batchStack.baseUrl + '/api/video/images', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
      body:JSON.stringify({ data:tinyPngBase64 }),
    });
    let shot3Name = (await json(shot3Upload)).name;
    let batchCreate = await fetch(batchStack.baseUrl + '/api/video/batches', {
      method:'POST',
      headers:{ 'content-type':'application/json' },
      body:JSON.stringify({
        modelId:'minimax-h3',
    ...
    let createdBatch = (await json(batchCreate)).batch;
    ...
    let batchComfyState = await json(await fetch(batchStack.upstreams.comfy.url + '/__mock/state'));
    let shotPrompts = batchComfyState.calls.filter(function (call: { path: string; body: { prompt: { [x: string]: { class_type: string; }; }; }; }) {
      return call.path === '/prompt' && call.body && call.body.prompt
    ...
    let shot2Call = shotPrompts[1].body.prompt;
    ...
    let shot3Call = shotPrompts[2].body.prompt;
    ...
    // 重抽语义：只有失败/取消镜可重抽。
    let retryOk = await fetch(batchStack.baseUrl + '/api/video/batches/' + createdBatch.id + '/shots/2/retry', {
      method:'POST',
    });
