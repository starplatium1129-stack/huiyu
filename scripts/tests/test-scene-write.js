...
test('applySceneChanges 修改留在原文件，其他分片字节不变', () => {
  ...
  assert.deepEqual(readShard('nene-core.1.json').find((s) => s.id === 'sc002').story, '修改后的故事');
  for (const [file, content] of Object.entries(before)) {
...
test('applySceneChanges 展示分类改名不迁移物理分组（After_Story 文字）', () => {
  ...
  const changes = sceneWrite.applySceneChanges(incoming, previous, { retiredIds: new Set() });
  assert.deepEqual(changes.touchedFiles, ['nene-core.1.json']);
  assert.ok(readShard('nene-core.1.json').some((s) => s.id === 'sc001'));
});
...
test('applySceneChanges char 归属变化才跨组移动', () => {
  ...
  assert.equal(readShard('nene-core.1.json').some((s) => s.id === 'sc001'), false);
  assert.equal(readShard('shared.json').some((s) => s.id === 'sc001'), true);
});
...
test('applySceneChanges 新增追加到组内最后批次，不满批不新开文件', () => {
  ...
  assert.deepEqual(readShard('nene-core.2.json').map((s) => s.id), ['sc004', 'sc005']);
});
...
test('applySceneChanges 批次满员新开下一批次', () => {
  ...
  assert.deepEqual(readShard('nene-core.2.json').map((s) => s.id), ['sc004', 'sc005', 'sc006']);
  assert.deepEqual(readShard('nene-core.3.json').map((s) => s.id), ['sc007']);
});
...
test('applySceneChanges 单文件组满批升级为批次形态', () => {
  ...
  assert.deepEqual(readShard('shared.1.json').map((s) => s.id), ['sc020', 'sc021', 'sc022']);
  assert.deepEqual(readShard('shared.2.json').map((s) => s.id), ['sc023']);
  // 升级后的目录仍是完整可读数据
...
test('applySceneChanges 下架清空批次保留空文件，后续分片不被截断', () => {
  ...
  assert.deepEqual(readShard('nene-core.2.json').map((s) => s.id), ['sc004']);
  assert.equal(loadPrevious().scenes.length, incoming.length);
});
...
test('retireRemovedScenes 登记退役并清样张', () => {
  ...
  const io = {
    readJson: (source) => JSON.parse(fs.readFileSync(source, 'utf8')),
    writeJson: (source, data) => fs.writeFileSync(source, JSON.stringify(data, null, 2) + '\n'),
    sanitizeCuration: (value) => value,
  };
...
test('cleanOrphanedSceneRefs 清理失效引用且保留无关设置', () => {
  ...
  sceneWrite.cleanOrphanedSceneRefs({
    ...
    io: {
      readJson: (source) => JSON.parse(fs.readFileSync(source, 'utf8')),
      writeJson: (source, data) => fs.writeFileSync(source, JSON.stringify(data, null, 2) + '\n'),
      sanitizeCuration: (value, activeIds) => {
        const curation = JSON.parse(JSON.stringify(value));
        curation.curatedSceneIds = curation.curatedSceneIds.filter((id) => activeIds.has(id));
