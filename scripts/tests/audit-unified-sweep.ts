...
let inspect: typeof import('../maintenance/image-inspect.js') = require('../maintenance/image-inspect.js');

let ROOT = path.resolve(__dirname, '..', '..');
let AI_ROOT = path.resolve(ROOT, '..', 'AI');
let DEFAULT_MANIFEST = path.join(AI_ROOT, 'Reviews', 'AnimaUnifiedSweep', '2026-08-13_24s_cfg3', 'manifest.json');

// 角色身份锚点：审核「身份特征还原」维度的判定基准（expectFor 使用）。
// --character natsume 切换为夏目；不传默认宁宁（向后兼容）。
let CHARACTER_EXPECT: any = {
  nene: '绫地宁宁（ayachi_nene，白发/紫瞳/呆毛/粉色发带，宁宁）',
  natsume: '四季夏目（shiki_natsume，黑色长直发/金黄瞳/红色发夹/泪痣，夏目）',
};
let character = ''; // 由 main() 从 --character 解析（模块级，expectFor 使用）

let AUDIT_PROMPT = inspect.TASKS.audit;
let MODEL = inspect.DEFAULT_MODEL;
...
function httpJson(urlPath: string, body: any, timeoutMs: number) {
  return new Promise(function (resolve, reject) {
    let url = new URL(inspect.DEFAULT_BASE_URL + urlPath);
    ...
    let req = http.request({
      ...
      res.on('end', function () {
        ...
        let detail = json && json.error ? JSON.stringify(json.error) : d.slice(0, 600);
...
async function withCooldownRetry(fn: any, maxAttempts?: number|undefined) {
  let attempts = maxAttempts || 5;
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      return await fn();
    } catch (e: any) {
      let message = String((e && e.message) || e);
      let cooldown = message.match(/reset_seconds"?\s*[:=]\s*"?(\d+)/);
      let waitSeconds = cooldown ? Number(cooldown[1]) + 5 : (/HTTP 5\d\d/.test(message) || /timeout/.test(message) || /ECONN|EPIPE|ETIMEDOUT/.test(message) ? 3 : 0);
      if (attempt >= attempts - 1 || waitSeconds <= 0) throw e;
      process.stderr.write('[冷却等待 ' + waitSeconds + 's] ' + message.slice(0, 100) + '\n');
      await new Promise<void>(function (resolve) { setTimeout(resolve, waitSeconds * 1000); });
    }
  }
}
...
async function auditImage(imagePath: string, expectText: string, prompt: string) {
  ...
  try {
    let j = await withCooldownRetry(function () {
...
function parseGroupVerdicts(content: string|null, count: number) {
  ...
  for (let i = 1; i <= count; i++) {
    let re = new RegExp('图片\\s*' + i + '\\s*[:：]', 'i');
    let line = lines.find(function (l) { return re.test(l); });
    if (!line) { verdicts.push({ verdict: 'parse-fail', score: null }); continue; }
    let m = line.match(/[:：]\s*(通过|需复核|不通过)/);
    let v = m && m[1] ? m[1] : 'parse-fail';
...
function expectFor(scene: any, record: any) {
  let parts = ['场景：' + (scene.title || record.sceneId)];
...
async function main() {
  ...
  let manifestFile = args.find(function (a) { return a.endsWith('.json'); }) || DEFAULT_MANIFEST;
  let outDirIndex = args.indexOf('--out-dir');
  let outDir = outDirIndex >= 0 ? args[outDirIndex + 1] : path.join(path.dirname(manifestFile), 'audit');
  let resume = args.includes('--resume');
  let characterIndex = args.indexOf('--character');
  ...
  let stageIndex = args.indexOf('--stage');
  let stage = stageIndex >= 0 ? args[stageIndex + 1] : 'both';
  if (!['both', 'quick', 'full'].includes(stage)) {
    console.error('--stage 只支持 quick|full|both（默认 both）');
    process.exit(2);
  }
  let batchIndex = args.indexOf('--batch');
  let batchSize = batchIndex >= 0 ? Number(args[batchIndex + 1]) : 6;
  if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 20) {
    console.error('--batch 必须是 1..20 的整数（默认 6）');
    process.exit(2);
  }
  let concIndex = args.indexOf('--concurrency');
  let concurrency = concIndex >= 0 ? Number(args[concIndex + 1]) : 3;
  ...
  let scenes = new Map(manifest.scenes.map(function (s: any) { return [s.id, s]; }));
  let records = manifest.records.filter(function (r: { status: string; }) { return r.status === 'succeeded'; });
  console.log('审核对象: ' + records.length + ' 张（' + manifest.paramGroup + '），stage=' + stage + '，batch=' + (stage === 'full' ? 1 : batchSize) + '，并发=' + concurrency);

  fs.mkdirSync(outDir, { recursive:true });
  let reportFile = path.join(outDir, 'audit-report.json');
  let report = resume && fs.existsSync(reportFile) ? readJson(reportFile) : { manifest:manifestFile, paramGroup:manifest.paramGroup, stage:stage, results:[] };
  ...
  if (stage === 'both' || stage === 'quick') {
    let quickTodo = records.filter(function (r: any) {
      let existing = findResult(r);
      return !existing || existing.quickVerdict === undefined;
    });
    console.log('[quick] 待快筛: ' + quickTodo.length + ' 张（每批 ' + batchSize + ' 张合并一次请求）');

    let quickBatches: any = [];
    for (let qi = 0; qi < quickTodo.length; qi += batchSize) {
    ...
    async function quickWorker() {
      ...
      while (true) {
        ...
        let batch = quickBatches[bi];
        let items = batch.map(function (r: any) {
          let scene = scenes.get(r.sceneId) || { title:r.sceneTitle || r.sceneId };
          return {
        ...
        let res = await quickCall(items);
        results.push({ index: bi, batch: batch, res: res });
      }
    }
    let quickWorkers = [];
    for (let qw = 0; qw < Math.min(concurrency, quickBatches.length || 1); qw += 1) quickWorkers.push(quickWorker());
    let quickCollected = (await Promise.all(quickWorkers)).flat();
    quickCollected.sort(function (a, b) { return a.index - b.index; });
    quickCollected.forEach(function (item) {
      let batch = item.batch;
      let res = item.res;
      let verdicts = res.ok ? parseGroupVerdicts(res.content, batch.length) : [];
      for (let vi = 0; vi < batch.length; vi++) {
        let r = batch[vi];
        let scene: any = scenes.get(r.sceneId) || { title:r.sceneTitle || r.sceneId };
        let existing = findResult(r);
        let entry = existing || {
  ...
  if (stage === 'both' || stage === 'full') {
    let fullTodo = records.filter(function (r: any) {
      let existing = findResult(r);
    ...
    async function fullWorker() {
      ...
      while (true) {
        ...
        let r = fullTodo[fi];
        let scene = scenes.get(r.sceneId) || { title:r.sceneTitle || r.sceneId };
        let imagePath = path.join(path.dirname(manifestFile), r.image);
        if (!fs.existsSync(imagePath)) {
          console.error('[缺失] ' + imagePath);
          continue;
        }
        process.stderr.write('[full ' + (fi + 1) + '/' + fullTodo.length + '] ' + r.candidate + ' ' + r.sceneId + ' seed-' + r.seed + ' → ' + MODEL + '\n');
        let res = await auditImage(imagePath, expectFor(scene, r), AUDIT_PROMPT);
        results.push({ index: fi, record: r, res: res });
      }
    }
    let fullWorkers = [];
    for (let fw = 0; fw < Math.min(concurrency, fullTodo.length || 1); fw += 1) fullWorkers.push(fullWorker());
    let fullCollected = (await Promise.all(fullWorkers)).flat();
  ...
  let order = Object.keys(byCandidate).sort(function (a, b) { return byCandidate[a].epoch - byCandidate[b].epoch; });

  let md = '# Anima Unified Sweep 审核汇总\n\n- manifest: `' + manifestFile + '`\n- 参数组: ' + manifest.paramGroup + '\n- 审核模型: ' + MODEL + '\n- 阶段: ' + stage + '\n\n';
  md += '## 各候选汇总（通过 / 需复核 / 不通过 / 失败，平均分满分 80）\n\n';
  md += '| 候选 | epoch | 通过 | 需复核 | 不通过 | 失败 | 平均分 |\n|---|---|---|---|---|---|---|\n';
  order.forEach(function (id) {
    let b = byCandidate[id];
    let avg = b.scores.length ? (b.scores.reduce(function (a: any, c: any) { return a + c; }, 0) / b.scores.length).toFixed(1) : '—';
  ...
  let mdFile = path.join(outDir, 'audit-report.md');
