...
const { fixture } = require('./content-check-fixture');
const { git, snapshot } = require('./content-history-fixture');
const { parse, checkContentImpact, main } = require('../maintenance/check-content-impact');
const script = path.resolve(__dirname, '../maintenance/check-content-impact.js');
const run = (f, ...args) => checkContentImpact(parse(['--root', f.root, '--base', f.base, ...args]));
...
test('deletion and rename use old relations, then execute full checks that find dangling references', (t) => {
  ...
  assert.ok(result.history.history.entities.some((item) => item.id === 'a' && item.change === 'removed'));
...
test('full structural fallback also has zero write/process effects; ownership describes field checks without claiming execution', (t) => {
  ...
  assert.equal(result.execution.status, 'passed-scoped');
  const ownership = require('../maintenance/report-content-ownership').reportOwnership({ root: f.root, domain: 'blueprints' }).domains[0];
  assert.equal(ownership.fieldValidation.status, 'not-run');
  assert.ok(ownership.fieldValidation.rules.some((rule) => rule.includes('parseSceneBlueprint')));
...
test('execution never invokes a builder or write API and checks only temporary data', (t) => {
  ...
  const proc = t.mock.method(cp, 'spawnSync', (command, args, options) => {
