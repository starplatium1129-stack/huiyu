...
const { VERSIONED_FILES } = require('../lib/data-version');
...
async function serve(rootDir, mode) {
  ...
  const { maintenanceReadToken } = require('../lib/maintenance-lease');
  const { createMaintenanceRouter, _test: helpers } = require('../../routes/maintenance');
  ...
  if (mode === 'startup') {
    ...
    catch (error) { process.send({ started: false, code: error.code }, () => process.exit(0)); }
    return;
  }
  if (!cfg.DESKTOP_PACKAGED) maintenanceReadToken(leaseOptions);
  const store = require('../lib/scene-store');
  const { captureMaintenanceSnapshot } = require('../lib/maintenance-transaction-snapshot');
  const { registerSceneMaintenance } = require('../../routes/maintenance-scene-save');
  const { maintenanceReadBarrier } = require('../../routes/maintenance-read-barrier');
  const app = express();
  ...
  app.use(require('../../server/precompressed').precompressed(rootDir));
  ...
  registerSceneMaintenance({ router, cfg, sceneStore: store, localOnly: helpers.maintenanceLocalOnly,
    ...
    syncVersion: root => {
      const version = require('../lib/data-version').expectedDataVersion(root);
