...
let SD = process.env.SD_HOST || 'http://127.0.0.1:7860';
let OUTPUT_ROOT = process.env.AICS_NATSUME_V19_MATRIX_DIR || path.join(
  AI_ROOT, 'Reviews', 'AnimaNatsumeV19ProductMatrix', '2026-08-09'
);
let LORA_ROOT = path.join(AI_ROOT, 'ComfyUI', 'models', 'loras');
let TRAIN_SAVE = path.join(AI_ROOT, 'OneTrainer', 'workspace', 'shiki_natsume_v19_anima_scientific_a', 'save');
let V18_SOURCE = path.join(AI_ROOT, 'Data', 'Models', 'Lora', 'shiki_natsume_v18_wd14.safetensors');
let SEEDS = [20260809, 20260810, 20260811];
let WIDTH = 1216;
let HEIGHT = 832;
let SCENES = [
...
let CLIENT_ID = 'aics-natsume-v19-product-matrix-' + crypto.randomUUID();
...
function writeJson(file: string, value: any) { fs.mkdirSync(path.dirname(file), { recursive:true }); fs.writeFileSync(file, JSON.stringify(value, null, 2) + '\n', 'utf8'); }
...
function sha256File(file: string) { return sha256(fs.readFileSync(file)); }
...
async function jsonRequest(base: string, pathname: string, options: RequestInit|undefined) {
  let response = await fetch(localUrl(base, pathname), options);
  let text = await response.text();
  let data = text ? JSON.parse(text) : null;
  if (!response.ok) throw new Error(pathname + ' returned HTTP ' + response.status + ': ' + text.slice(0, 500));
  return data;
}
function copyCandidate(candidate: any) {
  if (!candidate.epoch) return candidate;
  let match = fs.readdirSync(path.join(TRAIN_SAVE)).find(function (name) {
  ...
  let target = path.join(LORA_ROOT, candidate.file);
...
function promptFor(scene: any, engine: string) {
  ...
  let safety = scene.safe ? 'safe' : 'explicit, natsume_r18';
...
async function generateSd(scene: { id: string; label: string; safe: boolean; prompt: string; }, seed: number) {
  let data = await jsonRequest(SD, '/sdapi/v1/txt2img', { method:'POST', headers:{ 'Content-Type':'application/json' }, body:JSON.stringify({
  ...
  let encoded = String(data.images[0]).replace(/^data:image\/[^;]+;base64,/i, '');
...
function workflow(candidate: any, scene: { id: string; }, seed: string|number) {
  let nodes: any = {
...
async function waitFor(promptId: string|number) {
  ...
  while (Date.now() < deadline) {
    let history = await jsonRequest(COMFY, '/history/' + encodeURIComponent(promptId), { cache:'no-store' });
    let entry = history && history[promptId];
    if (entry) {
      let failed = (entry.status && entry.status.messages || []).find(function (message: string[]) { return message && message[0] === 'execution_error'; });
      if (failed) throw new Error('ComfyUI execution failed: ' + JSON.stringify(failed));
      let image = entry.outputs && entry.outputs['10'] && entry.outputs['10'].images && entry.outputs['10'].images[0];
...
async function generateAnima(candidate: any, scene: { id: string; label: string; safe: boolean; prompt: string; }, seed: number) {
  let submitted = await jsonRequest(COMFY, '/prompt', { method:'POST', headers:{ 'Content-Type':'application/json' }, body:JSON.stringify({ prompt:workflow(candidate, scene, seed), client_id:CLIENT_ID }) });
  assert(submitted && submitted.prompt_id, 'ComfyUI returned no prompt id');
  let image = await waitFor(submitted.prompt_id);
  let query = new URLSearchParams({ filename:image.filename, subfolder:image.subfolder || '', type:image.type || 'output' });
  let response = await fetch(localUrl(COMFY, '/view?' + query.toString()));
...
async function main() {
  ...
  let manifestFile = path.join(OUTPUT_ROOT, 'manifest.json');
  let manifest = fs.existsSync(manifestFile) ? readJson(manifestFile) : { version:1, purpose:'Natsume Anima v19 product matrix: WAI v18 production versus Anima base-only and scientific checkpoints', scenes:SCENES, seeds:SEEDS, candidates:candidates, records:[] };
  ...
  for (let scene of SCENES) for (let seed of SEEDS) {
    let key = function (candidateId: string, engine: string) { return candidateId + ':' + scene.id + ':' + seed + ':' + engine; };
    let baselineFile = path.join(OUTPUT_ROOT, imagePath({ id:'wai_v18' }, scene, seed, 'wai_v18'));
    let sdRecord = manifest.records.find(function (item: { key: string; }) { return item.key === key('wai_v18', 'sd'); });
    if (!sdRecord || !fs.existsSync(baselineFile)) {
      let sd = await generateSd(scene, seed); fs.mkdirSync(path.dirname(baselineFile), { recursive:true }); fs.writeFileSync(baselineFile, sd.body);
  ...
  for (let candidate of candidates) for (let scene2 of SCENES) for (let seed2 of SEEDS) {
    let animaKey = candidate.id + ':' + scene2.id + ':' + seed2 + ':anima';
    let animaRelative = imagePath(candidate, scene2, seed2, 'anima'); let animaFile = path.join(OUTPUT_ROOT, animaRelative);
    ...
    let anima = await generateAnima(candidate, scene2, seed2); fs.mkdirSync(path.dirname(animaFile), { recursive:true }); fs.writeFileSync(animaFile, anima.body);
