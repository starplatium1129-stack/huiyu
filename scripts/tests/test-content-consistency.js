...
const { reportOwnership } = require('../maintenance/report-content-ownership');
...
const { fixture, snapshot } = require('./content-history-fixture');
const inspect = (f, domain) => summarizeConsistency(inspectDomain(localReader(f.root), domain));
...
test('scene group placement, numeric sc1000 ordering, core and index projections are checked without builders', (t) => {
  ...
  for (const file of ['data/scenes-nene.json', 'data/scenes-core.json', 'data/scenes-index.json']) assert.ok(result.issues.some((r) => r.file === file));
});
...
test('changed curation updates expected core order; source gaps and orphan batches invalidate proof', (t) => {
  ...
  assert.ok(result.unknown.some((r) => r.includes('batch gap')));
  assert.ok(!result.checks.some((r) => r.status === 'current'));
});
...
test('reference identity/default/prose/perspective fields are mirrored; pending and URLs stay unverified', (t) => {
  ...
  assert.ok(result.checks[0].untracked.some((r) => r.includes('URL')));
  ...
  for (const suffix of ['/identityProse', '/isDefault', '/lens']) assert.ok(mismatch.issues.some((r) => r.location.endsWith(suffix)));
  assert.ok(!mismatch.issues.some((r) => r.location.includes('url')));
});
