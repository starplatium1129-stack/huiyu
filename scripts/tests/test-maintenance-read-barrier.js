...
const { precompressed } = require('../../server/precompressed');
...
const { createFixture, spawnWorker, tree } = require('./maintenance-recovery-fixture');
...
async function application(f, position) {
...
test('actual gateway early barrier protects gzip/br and showcase across a real writer SIGKILL', async () => {
  ...
  try {
    ...
    stack = await require('./gateway-test-stack').start({
    ...
    const request = (url, encoding, headers = {}) => new Promise((resolve, reject) => {
