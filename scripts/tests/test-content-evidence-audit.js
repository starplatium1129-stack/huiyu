...
const { fixture } = require('./content-evidence-fixture');
const { snapshot } = require('./content-history-fixture');
const { auditContentEvidence: audit } = require('../lib/content-evidence-audit');
const { parse, main } = require('../maintenance/audit-content-evidence');
...
test('unlisted source paths and unknown generator mappings remain unknown and are never read or executed', (t) => {
  ...
  const guard = t.mock.method(fs, 'readFileSync', (file, ...args) => {
...
test('escaping asset symlinks are rejected without reading their targets', (t) => {
  ...
  catch (error) { if (error.code === 'EPERM') { t.skip('file symlink unavailable; junction test below is unconditional'); return; } throw error; }
  const read = fs.readFileSync;
  t.mock.method(fs, 'readFileSync', (file, ...args) => { assert.notEqual(file, outside); return read(file, ...args); });
  const result = audit(f.options);
...
test('audit uses no write/process/network operations, and CLI prints matching statuses and exit codes', (t) => {
  ...
  const http = t.mock.method(require('node:http'), 'request', () => { throw new Error('No network allowed'); });
