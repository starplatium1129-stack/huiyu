...
let PROBE_ROOT = path.join(ROOT, '..', 'AI', 'Reviews', 'PopularProbe', '2026-08-14');
let MANIFEST = path.join(PROBE_ROOT, 'manifest.json');
...
async function auditImage(imagePath: string, expectText: string, strict: boolean) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      let j = await inspect.chatCompletion([{ role: 'user', content: content }], { maxTokens: strict ? 4000 : 1200, timeoutMs: 180000 }, MODEL);
      let text = j.choices && j.choices[0] && j.choices[0].message
        ? String(j.choices[0].message.content) : JSON.stringify(j).slice(0, 600);
      return { ok: true, content: text };
    } catch (e) {
      lastErr = e;
      let retriable = /HTTP 5\d\d/.test(runtimeErrorMessage(e)) || /超时/.test(runtimeErrorMessage(e)) || /ECONN|EPIPE|ETIMEDOUT/.test(runtimeErrorMessage(e));
      if (!retriable) break;
    }
  }
...
function parseResult(content: string|null, strict: boolean) {
  ...
  let mm = raw.match(/识别结果\s*\*{0,2}\s*[：:]\s*\*{0,2}\s*(是|否|存疑)/);
  let scene = raw.match(/场景符合度\s*\*{0,2}\s*[：:]\s*\*{0,2}\s*(符合|部分|不符合)/);
  return {
...
async function main() {
  ...
  let manifest = readJson(MANIFEST);
  ...
  let outDir = path.join(PROBE_ROOT, 'audit');
  fs.mkdirSync(outDir, { recursive: true });
  let reportFile = path.join(outDir, strict ? 'strict-report.json' : 'recognition-report.json');
  let report = fs.existsSync(reportFile) ? readJson(reportFile) : { manifest: MANIFEST, model: MODEL, strict: strict, results: [] };
  // 离线重解析：修复正则后对已有 parse-fail 记录直接重解析，不重新请求。
  let repaired = 0;
  report.results.forEach(function (x: any) {
    if (x.verdict !== 'parse-fail' || !x.content) return;
    let parsed = parseResult(x.content, strict);
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    let imagePath = path.join(PROBE_ROOT, r.image);
    if (!fs.existsSync(imagePath)) { console.error('[缺失] ' + imagePath); continue; }
    let character = byId[r.characterId] || { identityProse: r.characterId };
    let bp = bpByChar[r.characterId];
    process.stderr.write('[' + (i + 1) + '/' + records.length + '] ' + r.engine + ' ' + r.characterId + ' → ' + MODEL + '\n');
    let expect = '角色：' + (character.displayName || r.characterId) + '（' + (character.franchise || '') + '）\n' +
      '身份描述：' + (character.identityProse || '') + '\n' +
      '预期场景：' + (bp ? bp.title + '（' + bp.promptProse + '）' : '（无）');
    let res = await auditImage(imagePath, expect, strict);
    if (!res.ok) {
      ...
      let parsed = parseResult(res.content, strict);
      report.results.push({
  ...
  // 汇总
  let md = '#' + (strict ? 'Krea 2 热门角色严格八维审核' : 'Krea 2 热门角色识别审核') + '\n\n- 模型: ' + MODEL + '\n- 图片: ' + PROBE_ROOT + '\n\n';
  md += '## 结果\n\n| 角色 | 结论' + (strict ? '（总分/80）' : '') + ' | 备注 |\n|---|---|---|\n';
  let stats: any = {};
  report.results.forEach(function (x: any) {
    let label = x.ok ? (x.verdict || 'parse-fail') : '失败';
    stats[label] = (stats[label] || 0) + 1;
    let scoreLabel = x.sceneFit !== null && x.sceneFit !== undefined ? String(x.sceneFit) : '-';
    let firstLine = x.ok ? String(x.content || '').split('\n').filter(function (l) { return l.trim(); })[0] || '' : x.error;
  ...
  let mdFile = path.join(outDir, strict ? 'strict-report.md' : 'recognition-report.md');
