 ...
 * 不代表图片质量、审核或部署完成。实现见 scripts/lib/resource-pack.js。
 *
 * 退出码契约：
 *   0  成功（预览计划可行 / 候选包已通过字节核验并落盘）
 *   1  目标或内容问题（非 Windows apply、清单核验失败/格式错误/不支持 schemaVersion、目标已存在、
 *      目标祖先链含链接、复制或核验失败、发布后验收失败）
 *   2  参数或环境问题（包名非法、参数缺失或无法识别、root 不可用、清单路径越界或不可读）
 */

const path = require('node:path');
const { UsageError, planResourcePack, stageResourcePack } = require('../lib/resource-pack');

const HELP = [
  'resource:pack [options] — 离线资源候选包暂存导出（受控复制；不是安装器或下载器）',
  '  --manifest <root内JSON>   必填：源清单（先复用 audit:resource-manifest 同套核验，',
