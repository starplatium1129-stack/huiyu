...
function buildModernInstaller({ payload, output, preview = false, capture = false, theme = 'dark', state = 'ready', dpi = 96 } = {}) {
...
`;
  ...
  const xaml = fs.readFileSync(path.join(SOURCE, 'Installer.xaml'), 'utf8').replace(/@ICON_([A-Z]+)@/g, (_, key) => {
  ...
  const references = ['System.dll', 'System.Core.dll', 'System.Xaml.dll', 'System.Windows.Forms.dll'].map(file => path.join(framework, file));
  references.push(...['WindowsBase.dll', 'PresentationCore.dll', 'PresentationFramework.dll', 'UIAutomationTypes.dll'].map(file => path.join(framework, 'WPF', file)));
  const args = ['/nologo', '/utf8output', '/codepage:65001', '/target:winexe', '/platform:x64', '/optimize+', `/out:${executable}`, `/win32manifest:${path.join(SOURCE, 'app.manifest')}`, `/win32icon:${path.join(BASE, '../icons/icon.ico')}`,
    ...references.map(file => `/reference:${file}`), `/resource:${xamlFile},Installer.xaml`, `/resource:${path.join(BASE, 'atelier-keyart.png')},Keyart.png`, metaFile,
    ...['Program.cs', 'InstallEngine.cs', 'FolderPicker.cs', 'InstallerWindow.cs'].map(file => path.join(SOURCE, file))];
...
if (require.main === module) {
  ...
  const value = (key: string|unknown[], fallback: string|undefined) => args.find(arg => arg.startsWith(`--${key}=`))?.slice(key.length + 3) || fallback;
  ...
  else try { buildModernInstaller({ preview: args.includes('--preview'), capture: args.includes('--capture'), theme: value('theme', 'dark'), state: value('state', 'ready'), dpi: Number(value('dpi', '96')), payload: value('payload'), output: value('output') }); }
