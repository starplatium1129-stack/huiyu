...
function parse(argv) {
  const opts = { paths: [], showcaseManifests: [], root: path.resolve(process.env.AICS_DATA_ROOT || process.env.AICS_APP_ROOT || path.join(__dirname, '../..')) };
...
const isScenePath = (file) => file.startsWith('data/scenes/')
...
function sceneImpact(opts, result, add, recommend) {
  ...
  const read = (file) => JSON.parse(fs.readFileSync(localPath(file), 'utf8'));
  const groups = new Map();
  const rows = [];
  let complete = true;
  const sourceProblem = (file, reason) => {
    complete = false;
  ...
  try {
    ...
    for (const entry of manifest.files) {
      ...
      const batches = names.filter((name) => name.startsWith(`${base}.`) && /^\.\d+\.json$/.test(name.slice(base.length)))
        .sort((a, b) => Number(a.slice(base.length + 1, -5)) - Number(b.slice(base.length + 1, -5)));
      const group = { entry, files: batches.length ? batches : [entry.file], rows: [], ok: false };
  ...
  for (const file of opts.paths.filter(isScenePath)) {
    ...
    const group = logical || [...groups.values()].find((item) => item.files.some((name) => `${directory}${name}` === file));
    if (!group?.ok) {
      result.unknown.push(`${file}: 未解析路径、未登记/已删除/重命名或源不完整；不能从聚合推断影响边界`);
      continue;
    }
    const matches = logical ? group.rows : group.rows.filter((row) => row.file === file);
  ...
  const readMetadata = (file, validate, shape) => {
    try {
      const value = read(file);
      if (!validate(value)) throw new Error(shape);
      return value;
    } catch (error) {
      result.unknown.push(`${file}: 不可解析，相关状态 unknown（${error.message}）`);
      if (error.code !== 'ENOENT') add('mustChange', 'scene-metadata', file, error.message);
      return null;
    }
  };
  const aggregate = readMetadata(aggregateFile, (value) => Array.isArray(value) && value.every((scene) => scene && validId(scene.id)), '聚合须为含有效场景 ID 的数组');
  const curation = readMetadata(curationFile, (value) => value && typeof value === 'object' && !Array.isArray(value), '精选须为对象');
  const retired = readMetadata(retiredFile, (value) => Array.isArray(value?.records) && value.records.every((record) => record && validId(record.id)), '退役表须有含有效 ID 的 records 数组');
  const tiers = {};
  ...
  for (const [file, domain, ids] of [
    ...
    [retiredFile, 'retired-source', retired ? retired.records.map((record) => record.id) : null],
  ]) {
  ...
  for (const id of selected) {
    const sources = rows.filter((row) => row.value.id === id);
    const products = aggregate?.filter((scene) => scene.id === id);
    const records = retired?.records.filter((record) => record.id === id);
    const item = {
      id,
      sourceStatus: sources.length > 1 ? 'ambiguous' : !complete ? 'unknown' : sources.length ? 'present' : 'absent',
      sources: sources.map((row) => ({ file: row.file, group: row.group })),
    ...
    const productProblem = (status, reason) => {
    ...
    for (const [key, ids] of Object.entries(tiers)) {
      ...
      if (ids.filter((value) => value === id).length > 1) add('mustChange', 'curation', object, '目标场景在同一精选层重复登记');
    }
  }
  result.unknown.push('场景比较仅覆盖所选 ID 的数组聚合内容；未验证全域聚合顺序、按角色浏览器分片、核心/索引产物、精选审核真实性及其他场景引用域。');
}

function themeImpact(opts, selected, result, add) {
  ...
  try {
    ...
    const css = require('postcss').parse(readLocal(file), { from: file });
    selectors = new Set();
    css.walkRules((rule) => {
      const normalized = rule.selector.replace(/\[\s*data-character\s*=\s*(?:"([A-Za-z0-9_-]+)"|'([A-Za-z0-9_-]+)'|([A-Za-z0-9_-]+))\s*\]/g,
        (_, double, single, bare) => `[data-character="${double || single || bare}"]`);
    ...
    if (!css.nodes.some((node) => node.type === 'rule' || node.type === 'atrule')) throw new Error('CSS 无可解析规则');
  } catch (error) { cssError = `${file}: 不可解析（${error.message}）`; }
  try {
    const records = JSON.parse(readLocal('data/characters.json'));
    if (!Array.isArray(records) || records.some((row) => !row || !validId(row.id)) || new Set(records.map((row) => row.id)).size !== records.length) throw new Error('canonical 角色须为有效且不重复 ID 的数组');
    canonical = new Set(records.map((row) => row.id));
  } catch (error) { dataError = `data/characters.json: 不可解析（${error.message}）`; }
  result.themes = [...selected].map((id) => {
...
function outfitDefault(rows, id) {
  const item = { id, status: 'unknown', defaultOutfit: null, defaultIds: [], isDefaultIds: [], reasons: [] };
  ...
  const marked = fields.map((key) => outfits.filter((o) => o?.[key] === true));
  [item.defaultIds, item.isDefaultIds] = marked.map((list) => list.map((o) => o.id ?? null));
  const problems = [];
  if (marked.some((list) => list.length > 1)) problems.push(['ambiguous', '同一字段标记多个默认项']);
  if (marked.flat().some((o) => !validId(o.id))) problems.push(['missing', '默认项 ID 缺失或无效']);
  if (marked.flat().some((o) => validId(o.id) && outfits.filter((v) => v?.id === o.id).length > 1)) problems.push(['ambiguous', '默认 ID 在角色内重复，无法唯一解析']);
  if (outfits.some((o) => fields.every((key) => typeof o?.[key] === 'boolean') && o.default !== o.isDefault)) problems.push(['mismatch', 'default 与 isDefault 字段集合冲突']);
  const complete = outfits.length > 0 && outfits.every((o) => fields.every((key) => typeof o?.[key] === 'boolean'));
  if (complete && marked.some((list) => !list.length)) problems.push(['missing', '完整双字段中无默认项']);
  if (problems.length) {
    item.status = problems[0][0];
    item.reasons = problems.map(([, reason]) => reason);
...
function showcaseImpact(opts, result, add) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (path.posix.isAbsolute(normalized) || normalized.includes(':') || normalized.split('/').some((p) => p === '..' || !p)) throw new Error('manifest 必须为 root 内相对路径');
      const root = fs.realpathSync(opts.root);
      ...
      if (!object(manifest) || !Array.isArray(manifest.entries) || manifest.entries.some((entry) => !object(entry)
        || !validId(entry.id) || ['type', 'char'].some((key) => entry[key] !== undefined && typeof entry[key] !== 'string'))) throw new Error('manifest.entries 必须为含有效 id 与可解析 type/char 的对象数组');
      item.status = 'parsed';
      ...
      add('mustChange', 'showcase-manifest', file, `显式清单无法安全解析：${error.message}`);
      result.unknown.push(`${file}: 样张关系 unknown，不能推断缺失条目`);
    }
  }
  result.showcase.status = result.showcase.manifests.some((item) => item.status === 'error') ? 'error' : 'partial';
}
...
function report(opts) {
  ...
  const recommend = (name) => {
    ...
    if (!result.recommendations.some((r) => r.name === name)) result.recommendations.push({ name, argv: ['node', 'scripts/workflow.js', name], nature: def.run.nature, executed: false });
  };
  function shards(domain, key, aggregate, version) {
    try {
      ...
      try {
        if (!isDeepStrictEqual(read(aggregate), { version, [key]: rows.map((r) => r.value) })) {
          add('mustChange', domain, aggregate, '源/聚合内容失配（全域快照检查，未断言由本次输入造成）');
          recommend(domain === 'popular' ? 'popular:build' : 'blueprints:build');
        }
      } catch (error) { add('mustChange', domain, aggregate, `产物不可读取：${error.message}`); }
      return rows;
    } catch (error) {
      add('mustChange', domain, `data/${domain}`, error.message);
      result.unknown.push(`${domain} 源读取不完整，未从旧聚合推断关系`);
      return null;
    }
  }
  const characterScope = opts.character || opts.paths.some((file) => !isScenePath(file));
  ...
  for (const file of opts.paths) {
    ...
    const matches = [...(popular || []), ...(blueprints || [])].filter((r) => r.file === file);
  ...
  const byId = new Map((popular || []).map((r) => [r.value.id, r]));
  result.outfitDefaults = [...selected].map((id) => outfitDefault((popular || []).filter((r) => r.value.id === id), id));
  ...
  for (const id of selected) {
    ...
    if (ids.some((v) => typeof v !== 'string' || !v) || new Set(ids).size !== ids.length) add('mustChange', 'outfit', id, '服装 ID 缺失或角色内重复');
    ...
    for (const bp of blueprints || []) {
      ...
      if (!opts.outfit || bp.value.outfitId === opts.outfit || !bp.value.outfitId) {
        const defaultInfo = result.outfitDefaults.find((item) => item.id === id);
...
module.exports = { parse, report, main };
