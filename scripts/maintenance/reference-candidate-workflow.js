...
const { CODE_ROOT, hash, runCli } = require('../lib/generation-candidates');
...
function inspectedSummary(inspection) {
...
function reviewCandidates(options, inspection) {
...
async function main(args = process.argv.slice(2), deps = {}) {
  ...
  if (options.action === 'full') {
    const generator = require('./render-all-outfits-references');
...
module.exports = { main, parse, reviewCandidates };
