...
const { loadSceneShards }: typeof import('../lib/scene-store') = require('../lib/scene-store');
...
const MANUAL_SCENE_RATINGS: typeof import('../lib/manual-scene-ratings.js') = require('../lib/manual-scene-ratings.js');
...
function readJson(source: PathOrFileDescriptor, label: string, errors: string[]) {
  try {
    return JSON.parse(fs.readFileSync(source, 'utf8'));
  } catch (error) {
    errors.push(label + ' cannot be parsed: ' + runtimeErrorMessage(error));
    return [];
  }
}
...
try {
  ...
  errors.push('scene shards cannot be loaded: ' + runtimeErrorMessage(error));
}
