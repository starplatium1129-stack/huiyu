...
const { MAX_BYTES, safeRelative, assertWorktreePaths, reader }: typeof import('../lib/content-history-reader') = require('../lib/content-history-reader');
...
function collectGitHistory(root: PathLike, base: string) {
  ...
  try {
    ...
    result.reason = runtimeErrorMessage(error);
    result.changes = [];
    result.paths = [];
...
function collectGitChanges(root: PathLike) {
  ...
  } catch (error) { result.reason = runtimeErrorMessage(error); }
  return result;
}
export = { collectGitChanges, collectGitHistory, validateRevision };
