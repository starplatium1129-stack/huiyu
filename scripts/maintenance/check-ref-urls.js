...
const { resolveContentRoot } = require('../lib/content-contract-root');
...
function main(args = process.argv.slice(2)) {
  ...
  try {
    ...
    console.error('错误: 视图文件缺失或无法解析: ' + viewPath + '（' + error.message + '）');
    return 1;
  }
  ...
  try {
    ...
    console.error('错误: 视图内容无法审计: ' + viewPath + '（' + error.message + '）');
    return 1;
  }
...
module.exports = { auditReferenceView, main, HELP };
