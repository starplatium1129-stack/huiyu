...
function sceneImpact(opts, result, add, recommend) {
  ...
  const readMetadata = (file: string, validate, shape: string|undefined) => {
    try {
...
function themeImpact(opts: { root: PathLike; }, selected, result, add) {
...
function outfitDefault(rows, id) {
  ...
  const marked = fields.map((key) => outfits.filter((o: { [x: string]: boolean; }) => o?.[key] === true));
