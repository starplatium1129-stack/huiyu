...
function report(o) {
  ...
  if (o.comparisons?.length) {
    r.comparisons = o.comparisons.map(input => {
      ...
      } catch (e) { result.message = runtimeErrorMessage(e); add('errors', input, 'comparisons', runtimeErrorMessage(e)); }
      return result;
    });
  }
  let primary;
  try { primary = read(o.evidence); } catch (e) { add('errors', o.evidence, '$', runtimeErrorMessage(e)); return finish(); }
  const receipt = text(primary.d.evidence) && Object.hasOwn(primary.d, 'commit') ? primary : null;
  let evidence = primary;
  if (receipt) {
    try { evidence = read(receipt.d.evidence); } catch (e) { add('errors', receipt.file, 'evidence', runtimeErrorMessage(e)); }
  }
  ...
  if (delivery) {
    ...
    } catch (e) { add('errors', delivery.file, 'evidence', runtimeErrorMessage(e)); }
  }
