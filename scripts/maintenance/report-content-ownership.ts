...
const DOMAINS = {
  ...
  references: { fields: 'data/references 每人物 standard/view；manifest 共用机位、人物与 view 顺序', readers: ['scripts/lib/reference-store.js（读取源、确定性重建聚合）', 'src/utils/characterReferenceData.ts（按人物请求本机 API，已安装版本优先）', 'scripts/tests/test-character-reference-contract.js（standards/view 镜像契约）'], writers: ['人工维护 data/references/<人物ID>.json', 'reference:register / content:sync --apply（增量 pending 登记）', '旧库维护脚本合并写入后经 writeReferenceLibrary 同步分片与兼容聚合'], boundary: '两个 character-reference 聚合 JSON 不入 Git，不手工编辑；URL/pending 不证明资产交付或审核；旧库维护仍需核对真实素材，不能代替候选审核发布' },
  themes: { fields: 'data-character 运行时主题及 --character-* 变量：主题颜色令牌', readers: ['src/utils/characterTheme.ts（characters.json accent_color + 人工调校例外）', 'src/assets/css/director.css（@import 引入 tokens.css）', '消费 var(--character-*) 的样式与组件（chat.css、director/*、companion.css、light-theme.css 等）'], writers: ['人工维护 data/characters.json accent_color', '人工维护 src/utils/characterTheme.ts 中的既有角色调校目录'], boundary: '主题渲染由 runtime characterTheme.ts 注入 .pb，新增角色默认从 characters.json 派生；tokens.css 不再为角色枚举选择器；主题仍需双主题视觉与对比度验收' },
...
const FIELD_CONTRACTS: any = {
  ...
  references: { source: 'data/references/manifest.json + per-character shards', derived: ['data/character-reference-standards.json', 'data/character-reference-view.json'],
    rule: 'reference-store reconstructs standards and view preserving fields and declared order; registration writes pending source records then rebuilds compatible products',
...
function reportOwnership({ root = path.resolve(__dirname, '../..'), domain, consistency = false }: any = {}) {
  ...
  function inspect(file: any, role?: any, validate?: any, text: any = false) {
    ...
    try {
      ...
      row.reason = runtimeErrorMessage(error);
      value = undefined;
    }
    return { row, value };
  }
    ...
    if (['popular', 'scenes', 'blueprints'].includes(id)) {
      ...
      if (manifest) for (const entry of manifest.files) {
        ...
        if (id === 'scenes') {
          ...
          } catch (error) { result.entries.push({ path: directory, role: 'source', status: 'invalid', reason: runtimeErrorMessage(error), quality: 'unverified' }); files = []; }
        }
