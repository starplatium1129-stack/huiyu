...
const { rootPath, evidencePath, saveJson } = require('../lib/delivery-paths');
const { selectors } = require('../lib/delivery-identity');
const { gatePath } = require('../lib/delivery-state');
...
function main(args) {
  let options;
  try { options = parse(args); } catch (error) { console.error(error.message); return 2; }
