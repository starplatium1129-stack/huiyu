...
function resolveProfile() {
  ...
  const profile = (presets.model_profiles || []).find((item: any) => item.id === ANIMA_PROFILE_ID);
  if (!profile) throw new Error(`presets.json missing profile ${ANIMA_PROFILE_ID}`);
  return profile;
}
function nearestAnimaSize(blueprint: any) {
...
function buildCandidate(character: any, blueprint: any, profile: any, attempt: any, seedAttempt: any = attempt) {
  ...
  const blueprintNegative = blueprintNegativeRaw
    ...
    .filter((token: any) => token && (!adult || !adultNegativeBlockers.has(token.toLowerCase().replace(/_/g, ' '))))
    .join(', ');
...
function planAll(attempt: any, seedAttempt: any = attempt, keys: any = []) {
...
function canReuseCandidate(previous: any, candidate: any) {
  ...
  return previous?.status === 'succeeded' && fields.every((field: any) => previous[field] === candidate[field]);
}
async function submitCandidate(base: any, candidate: any) {
  ...
  for (let retry = 0; retry < 5; retry += 1) {
    ...
    if (!busy) break;
    await new Promise<any>((resolve: any) => setTimeout(resolve, 5000 * (retry + 1)));
  }
...
async function main() {
  ...
  const selected = keys.length ? planned.filter((c: any) => keys.includes(c.key)) : planned;
  if (keys.length && selected.length !== keys.length) {
    const found = new Set(selected.map((c: any) => c.key));
    throw new Error(`unknown keys: ${keys.filter((k: any) => !found.has(k)).join(', ')}`);
  }
  ...
  const pending = selected.filter((candidate: any) => {
  ...
  const normalized = [...records.values()].sort((left: any, right: any) =>
