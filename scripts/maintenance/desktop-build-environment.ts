...
function resolveSdkRoot(root: any = ROOT, env: NodeJS.ProcessEnv = process.env, exists: any = fs.existsSync): string {
...
function inspectDesktopBuildEnvironment(options: EnvironmentOptions = {}) {
  ...
  const windowsSdk = fs.existsSync(sdkLib) ? fs.readdirSync(sdkLib).find((version: any) => fs.existsSync(path.join(sdkLib, version, 'um/x64/kernel32.lib'))) : '';
  ...
  add('Cubism Native R5', sdkRoot && SDK_PARTS.every((part: any) => fs.existsSync(path.join(sdkRoot, part))), sdkRoot,
    '将官方 CubismSdkForNative-5-r.5 放在 runtime/desktop-build-sdk/，或设置 LIVE2D_CUBISM_SDK_DIR');
  return { ready: checks.every((check: any) => check.ready), checks, sdkRoot };
}

function assertDesktopBuildEnvironment(root: any = ROOT) {
  const report = inspectDesktopBuildEnvironment({ root });
  if (!report.ready) throw new Error('桌面构建环境未就绪：\n' + report.checks.filter((check: any) => !check.ready).map((check: any) => `${check.name}: ${check.help}`).join('\n'));
  return report;
}
