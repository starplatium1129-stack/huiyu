...
function buildManifest(scenes: any, sourceManifest: any, additions: any, context: any) {
  const _sceneById = new Map(scenes.map((scene: any) => [scene.id, scene]));
  const additionById = new Map(additions.map((item: any) => [item.entry.id, item.entry]));
  ...
  const nonScene = sourceEntries.filter((entry: any) => entry.type !== 'scene' && !incoming.has(entry.id));
  const entries = [
    ...additions.map((item: any) => item.entry).sort((a: any, b: any) => a.id.localeCompare(b.id)),
    ...nonScene,
  ];
...
function verifyTarget(tempDir: any, manifest: any) {
  ...
  try {
    ...
    throw new Error(`home-hero.json unreadable: ${runtimeErrorMessage(error)}`);
  }
}
...
async function main() {
  ...
  const coveredSceneIds = new Set(additions.map((item: any) => item.record.sceneId));
  const uncovered = scenes.filter((scene: any) => !coveredSceneIds.has(scene.id)).map((scene: any) => scene.id);
  if (uncovered.length) {
  ...
  const summary = {
    ...
    additions: additions.map((item: any) => item.entry.id),
