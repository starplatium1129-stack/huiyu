...
const { planBlueprintChanges, BlueprintChangePlanError }: typeof import('./blueprint-change-plan') = require('./blueprint-change-plan');
...
function derivePaths(rootReal: string) {
...
function requireRealDirectory(abs: string, io: typeof import("node:fs"), label: string) {
  let st;
  try {
    st = io.lstatSync(abs);
  } catch (error) {
    throw new BlueprintWriteError(label + ' 不可访问: ' + runtimeErrorMessage(error), 'boundary', abs);
  }
  ...
  try {
    ...
    throw new BlueprintWriteError(label + ' 真实路径无法解析: ' + runtimeErrorMessage(error), 'boundary', abs);
  }
...
function readRealFile(abs: string, io: typeof import("node:fs"), label: string, allowAbsent: boolean) {
  let st;
  try {
    st = io.lstatSync(abs);
  } catch (error) {
    if (allowAbsent && runtimeErrorCode(error) === 'ENOENT') return { exists: false, content: null };
  ...
  try {
    ...
    throw new BlueprintWriteError(label + ' 不可读取: ' + runtimeErrorMessage(error), 'boundary', abs);
  }
  return { exists: true, content };
}

/** 分片基名重验（与规划器一致）：拒绝 traversal/绝对路径/分隔符/保留名/设备名。 */
function assertSafeShardName(file: string, where: string) {
...
function shardAbsPath(shardsDir: string, file: string) {
...
function prepareBlueprintWrite({ rootDir, blueprints, franchiseByCharacter, io = nodeFs } = {}) {
  ...
  try {
    ...
    throw new BlueprintWriteError('rootDir 不可解析为真实路径: ' + runtimeErrorMessage(error), 'boundary', rootDir);
  }
...
function expectFileBytes(absPath: string, expected: Uint8Array<ArrayBufferLike>|Buffer<ArrayBuffer>, io: typeof import("node:fs"), phase: string, code?: string|undefined) {
  ...
  try {
    ...
    throw new BlueprintWriteError(phase + '读取 ' + absPath + ' 失败: ' + runtimeErrorMessage(error), code || 'readback', absPath);
  }
...
function applyBlueprintWrite(prepared: object, { writeFileAtomic, io = nodeFs } = {}) {
  ...
  const shardWrites = plan.writes.map((item: { kind: string; text: WithImplicitCoercion<string>; file: string; }) => {
  ...
  const shardDeletes = plan.deletes.map((item: { file: string; }) => {
  ...
  const writePaths = new Set(shardWrites.map((item: { abs: string; }) => item.abs.toLowerCase()));
  ...
  for (const entry of state.baseline) {
    if (entry.exists) {
      ...
      try {
        ...
        if (runtimeErrorCode(error) === 'ENOENT') continue;
        throw new BlueprintWriteError('基线复核：无法确认 ' + entry.file + ' 状态: ' + runtimeErrorMessage(error), 'stale', entry.file);
      }
  ...
  for (const item of shardDeletes) {
    try {
      ...
      throw new BlueprintWriteError('删除分片 ' + item.file + ' 失败: ' + runtimeErrorMessage(error), 'io-error', item.abs);
    }
  }
  ...
  const aggregateEntry = entries.find((entry: { file: string; }) => entry.file === derived.aggregatePath);
  ...
  for (const item of shardDeletes) {
    ...
    try {
      ...
      gone = runtimeErrorCode(error) === 'ENOENT';
      if (!gone) throw new BlueprintWriteError('写回核验：无法确认 ' + item.abs + ' 已删除: ' + runtimeErrorMessage(error), 'readback', item.abs);
    }
