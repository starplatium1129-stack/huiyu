...
const field = (row, key) => ((row && typeof row[key] === 'string' && row[key]) || '');
...
function referenceSection(items) {
  const list = rows(items);
  if (!list.length) return [];
  const lines = list.map((row) => {
...
function formatImpactReport(result) {
...
module.exports = { formatImpactReport };
