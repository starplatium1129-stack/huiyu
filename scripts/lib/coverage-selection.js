 ...
 *  不做自动改写。 */
function assertSelectionKnown(selection, known, aliasMap) {
...
function scopeFileExists(fileExists, scopedUrls, selection) {
  return (url, owner) => {
...
function describeScope(selection) {
  return {
...
function filterReportToScope(report, selection) {
  const { character, outfit } = selection;
  const inScope = (row) => row.id === character && (!outfit || row.outfitId === outfit);
  ...
  const scopedThemes = {
    explicit: themes.explicit.filter((id) => id === character),
    defaultAllowed: themes.defaultAllowed.filter((id) => id === character),
    missingTheme: themes.missingTheme.filter((row) => row.id === character),
    staleAlias: [],
    nonCharacter: [],
