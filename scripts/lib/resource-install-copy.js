...
const { child, noLinks, mkdir, ensureSpace, writeAll, event, cancelled, fail, digest, flushDir, unlink } = require('./resource-install-fs');

const CHUNK = 512 * 1024;
function fileMatches(ctx, file, entry) {
...
async function copyEntry(ctx, { sourceRoot, tree, parts, entry, signal }) {
  cancelled(signal);
...
module.exports = { CHUNK, fileMatches, copyEntry };
