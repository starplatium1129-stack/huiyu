...
const { inspectDomain, summarizeConsistency } = require('./content-impact-consistency');
const { localReader } = require('./content-history-reader');
const { PRODUCTS } = require('./content-history-snapshot');
const { relations, relationshipIssues } = require('./content-history-relations');
...
function selectExecution(report, forceFull = false, context) {
  const reasons = [];
  ...
  if (report) {
    ...
    const knownPaths = new Set((report.evidence?.files || []).map((entry) => entry.file));
    ...
    for (const change of report.history.entities) {
      if (!['blueprints', 'scenes'].includes(change.domain) || change.change !== 'modified'
        ...
        || change.changedFields.some((field) => !['/prompt', '/negative', '/animaCaption'].includes(field))
  ...
  const targets = (report?.affected || []).filter((row) => row.impact === 'revalidate').map((row) => row.key);
...
function recordEquality(snapshots, keys) {
  ...
  for (const key of keys) {
    ...
    const source = snapshot.rows.filter((row) => row.role === 'source' && row.key === key);
    ...
    for (const [group, data] of Object.entries(snapshot.groups).filter(([name]) => name.includes(':derived:'))) {
      ...
      if (domain === 'scenes' && file !== PRODUCTS.scenes) {
        ...
        count = file === 'data/scenes-core.json'
          ? snapshot.metadata['data/curation.json'].personaCoreSceneIds.slice(0, 2000).filter((v) => v === id).length
          : Number(file === `data/scenes-${owner}.json`);
      }
      const actual = data.rows.filter((row) => row.key === key);
      if (!data.complete) unknown.push(`${file}: incomplete projection`);
      else if (actual.length !== count || actual.some((row) => !equal(row.value, value))) issues.push({ key, file, reason: 'Selected source/derived record mismatch' });
    }
  }
  return outcome('record-equality', [...keys], issues, unknown);
}

function relationCheck(snapshots, keys) {
  const unknown = [];
  for (const snapshot of Object.values(snapshots)) {
    ...
    for (const row of snapshot.rows.filter((row) => row.role === 'source' && (!keys || keys.has(row.key)))) relations(row, snapshots, unknown);
  }
  return outcome('source-relationships', keys ? [...keys] : 'all supported source IDs', relationshipIssues(snapshots, { keys }), [...new Set(unknown)]);
}

function runtimeFieldChecks(snapshots, keys) {
  const issues = [], unknown = [], coverage = [];
  let parsers;
  try { parsers = require('../../src/utils/popularContent.ts'); }
  catch (error) { unknown.push(`Existing runtime parsers unavailable: ${error.message}`); }
...
function fullFieldChecks(reader, snapshots) {
  ...
  } catch (error) { checks.push(outcome('existing-core-field-contract', 'core fields', [], [error.message])); }
  for (const name of ['standards', 'view']) {
    try {
      ...
      checks.push(outcome(`existing-reference-${name}-schema`, 'all declared schema fields', valid ? [] : validate.errors.map((error) => ({
        path: error.instancePath, schemaPath: error.schemaPath, reason: error.message,
      }))));
    } catch (error) { checks.push(outcome(`existing-reference-${name}-schema`, 'reference fields', [], [error.message])); }
  }
  const unknown = [];
  for (const domain of ['characters', 'popular', 'blueprints', 'scenes']) {
    const fields = [...new Set((snapshots[domain]?.rows || []).filter((row) => row.role === 'source').flatMap((row) => Object.keys(row.value)))];
...
function executePredicates(selection, context, root) {
...
module.exports = { DOMAINS, UNTRACKED, FIELD_VALIDATION, selectExecution, executePredicates, recordEquality, relationCheck, fullFieldChecks, runtimeFieldChecks };
