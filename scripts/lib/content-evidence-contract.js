'use strict';
const { object, validId } = require('./content-history-reader');
const { SHA, jsonHash, evidencePath } = require('./content-evidence-io');
...
function validateRecord(record, marker) {
  if (!object(record) || record.schemaVersion !== 1 || record.runId !== marker.runId || record.generator !== marker.generator
    ...
    || !Array.isArray(record.sources) || !record.sources.length || record.sources.some((source) => !object(source)
  ...
  if (new Set(record.sources.map((source) => source.path)).size !== record.sources.length) throw new Error('Duplicate candidate source declarations');
...
function inputVersion(record) {
  ...
  const metadata = Object.fromEntries(keys.filter((key) => Object.hasOwn(record, key)).map((key) => [key, record[key]]));
...
function reviewState(record, integrity, decisionFile, manifestSha256, runId) {
...
module.exports = { UUID, METADATA, validateRecord, inputVersion, reviewState, validateDecisions };
