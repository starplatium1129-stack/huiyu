...
const { validatePng } = require('./generation-gateway');
...
function inspectCandidates({ from, root = CODE_ROOT }) {
  ...
  const items = [...latest.values()].sort((a, b) => a.key.localeCompare(b.key)).map(record => {
    ...
    } catch (error) { integrity = 'invalid'; reason = error.message; }
...
module.exports = { GENERATOR, SOURCE, SHA, bytes, json, within, relativeImage, jsonHash,
