...
const { inspectMaintenanceLease, claimRecovery } = require('./maintenance-lease');
const { readBackup, saveSnapshotBackup, restoreEntries } = require('./maintenance-recovery-backup');
...
function previewMaintenanceRecovery(options) {
  ...
  if (journal?.backup && state.status === 'stale') {
    ...
    } catch (error) { conflicts.push({ code: error.code, message: error.message }); }
  ...
  try {
    ...
    try {
      if (undo) claim.update({ phase: rollbackErrors.length ? 'INCONSISTENT' : 'recovery-failed', recovery: {
        pid: process.pid, nonce: claim.nonce, undo: { id: undo.id, sha256: undo.sha256 }, error: error.message,
        rollback: rollbackErrors.length ? 'INCONSISTENT' : 'pre-recovery-restored', rollbackErrors,
      } });
      claim.finish();
    } catch (journalError) { rollbackErrors.push(journalError.message); }
    return { ok: false, code: error.code || 'MAINTENANCE_RECOVERY_FAILED', error: error.message, rolledBack: rollbackErrors.length === 0,
...
module.exports = { previewMaintenanceRecovery, applyMaintenanceRecovery };
