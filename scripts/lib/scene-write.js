  ...
  for (const entry of manifest.files || []) {
    ...
    for (const file of groupFiles) {
      ...
      try {
        ...
        problems.push(file + ': 无法读取 (' + error.message + ')');
        continue;
      }
...
function groupFileOrder(entry, workingFiles) {
  const base = groupPrefix(entry.file);
...
function applySceneChanges(incoming, previous, options) {
  ...
  function appendToGroup(entry, scene) {
  ...
  function removeFrom(file, id) {
    const scenes = working.get(file);
    const index = scenes.findIndex((scene) => String(scene.id) === id);
  ...
  for (const scene of incoming) {
    ...
    if (!prev) {
      ...
      appendToGroup(previous.manifest.files.find((entry) => entry.file === store.targetFile(scene)), scene);
      addedIds.push(id);
      continue;
    }
 ...
 * 保持与原实现完全相同的落盘格式。
 */
function cleanOrphanedSceneRefs(options) {
 ...
 */
function retireRemovedScenes(options) {
  const { incomingScenes, previousScenes, rootDir, showcaseDir, io, log } = options;
  const incomingIds = new Set(incomingScenes.map((scene) => scene.id));
    ...
    added.forEach((sceneId) => {
      ...
      if (fs.existsSync(manifestPath)) {
        ...
        if (manifest && Array.isArray(manifest.entries)) {
          manifest.entries = manifest.entries.filter((entry) => entry.id !== sceneId);
...
function withSceneWriteLock(task) {
