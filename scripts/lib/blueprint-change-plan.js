...
function mappingLookup(mapping, key) {
...
function validateShards(shards, entries, problems) {
  let provided;
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      problems.push('分片 ' + file + ' 的 text 不是合法 JSON: ' + error.message);
      continue;
    }
...
function validateTarget(blueprints, mapping, mappingUsable, problems) {
...
function planWrite(file, franchise, group, kind) {
  const data = { version: 2, franchise, blueprints: group };
  return { file, franchise, count: group.length, kind, data, text: jsonText(data) };
}

function buildPlan(manifest, entries, shardMap, groups, problems) {
  const existingByFranchise = new Map(entries.map((entry) => [entry.franchise, entry]));
  const nextFiles = [];
  const writes = [];
  const deletes = [];
  const unchanged = [];
  ...
  const occupied = new Set(entries.map((entry) => entry.file.toLowerCase()));
 ...
 */
function planBlueprintChanges(input) {
