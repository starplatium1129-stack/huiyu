...
function validateRun(name: string, def: { run?: unknown; cmd?: string[] | null }) {
  ...
  for (const effect of nature) if (!EFFECTS.some(item => item === effect)) errors.push(`${name}: 未知 nature "${String(effect)}"（合法值见 EFFECTS）`);
  ...
  if (run.switches != null && typeof run.switches === 'object' && !Array.isArray(run.switches)) {
    for (const [flag, effects] of Object.entries(run.switches)) {
      ...
      for (const effect of values) if (!EFFECTS.some(item => item === effect)) errors.push(`${name}: 开关 ${flag} 未知行为 "${String(effect)}"`);
    }
  } else if (run.switches != null) {
    errors.push(`${name}: run.switches 必须是对象`);
  }
  if (!RESUME_MODES.some(mode => mode === run.resume)) errors.push(`${name}: resume 必须是 ${RESUME_MODES.join('|')} 之一`);
...
function expand(name: string, args: string[], registry: WorkflowRegistry, parents: string[] = []): WorkflowStep[] {
...
function plan(name: string, args: string[], registry: WorkflowRegistry): WorkflowStep[] {
  ...
  return [
    { name: 'showcase:generate', args: ['--output', output, ...['--gateway', '--keys', '--concurrency', '--limit'].flatMap(k => { const value = option(args, k); return value ? [k, value] : []; })] },
    { name: 'showcase:audit', args: ['--manifest', manifest, '--out', path.join(output, 'audit-results.json')] },
    { name: 'showcase:publish', args: ['--from', manifest, '--source', source, '--target', target, ...(() => { const value = option(args, '--showcase'); return value ? ['--showcase', value] : []; })()] },
  ].map(step => ({ ...step, def: registry[step.name] }));
}

function invocation(def: WorkflowDefinition, args: string[]): [string, string[]] {
  if (!def.cmd?.length) throw new Error('工作流缺少可执行命令');
...
function main(argv: string[], registry: WorkflowRegistry, root: string, run: WorkflowCommandRunner = spawnSync) {
  try {
    const [name, ...raw] = argv;
...
export = { main, plan, audit, invocation, validateRun, EFFECTS, MACHINES, RESUME_MODES, PASSIVE_EFFECTS };
