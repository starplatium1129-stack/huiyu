...
const { acquireMaintenanceLease } = require('./maintenance-lease');

function prepareMaintenanceTransaction(lease, options, snapshot, label = 'content') {
  lease.assertOwned();
...
function commitMaintenanceTransaction(lease) {
  lease.complete('committed');
  lease.release();
}
function rollbackMaintenanceTransaction(lease, options) {
  ...
  try {
    ...
    try { lease.markInconsistent(error.message); } catch { /* A lost or damaged lease stays blocked. */ }
    return { ok: false, error: error.message, dataIntegrity: 'INCONSISTENT' };
  }
}
async function withMaintenanceTransaction(options, capture, task, label = 'content') {
...
module.exports = { prepareMaintenanceTransaction, commitMaintenanceTransaction, rollbackMaintenanceTransaction, withMaintenanceTransaction };
