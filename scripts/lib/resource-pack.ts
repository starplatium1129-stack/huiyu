...
const { SCHEMA_VERSION, UsageError, verifyManifest, verifyManifestEntries, resolveRealpathBoundary, checkManifestPath }: typeof import('./resource-manifest') = require('./resource-manifest');
...
function validatePackName(name: string) {
...
function destinationAbsPath(rootReal: string, name: string) {
...
function checkDestination({ root, name, io = nodeFs }: any) {
  ...
  for (const seg of [...PACKS_DIR_SEGMENTS, name]) {
    ...
    try {
      ...
      if (runtimeErrorCode(err) === 'ENOENT') break; // 自此以下不存在；--apply 时按真实目录创建
      errors.push({ path: relFromRoot(rootReal, cur), code: 'destination-stat-error', message: `目标祖先不可访问: ${runtimeErrorMessage(err)}` });
    ...
    try {
      ...
      errors.push({ path: relFromRoot(rootReal, cur), code: 'realpath-error', message: `无法解析目标祖先真实路径: ${runtimeErrorMessage(err)}` });
      return { ok: false, rootReal, destAbs, errors };
    }
  ...
  try {
    ...
    if (runtimeErrorCode(err) !== 'ENOENT') errors.push({ path: relFromRoot(rootReal, destAbs), code: 'destination-stat-error', message: `无法确认目标状态: ${runtimeErrorMessage(err)}` });
  }
  if (destSt) {
...
function loadVerifiedManifest({ rootReal, manifestPath, io }: any) {
  ...
  try {
    ...
    throw new UsageError(`--manifest 不可读取: ${runtimeErrorMessage(err)}`);
  }
  let manifest;
  try {
    manifest = JSON.parse(raw.toString('utf8'));
  } catch (err) {
    return { ok: false, manifest: null, verification: null, manifestRel: rel, errors: [{ code: 'bad-manifest', message: `清单 JSON 解析失败: ${runtimeErrorMessage(err)}` }] };
  }
  ...
  try {
    ...
    if (errors.length === 0) {
      ...
      for (const file of metadataFiles) {
        try {
          ...
          errors.push({ code: 'write-error', message: `候选包 ${file.name} 写入失败: ${runtimeErrorMessage(err)}` });
          break;
        }
      }
    }
    if (errors.length === 0) {
      ...
      for (const file of metadataFiles) {
        ...
        try {
          ...
          errors.push({ code: file.mismatchCode, message: `磁盘 ${file.name} 不可解析: ${runtimeErrorMessage(err)}` });
          break;
        }
    ...
    if (errors.length === 0) {
      ...
      try {
        ...
        if (runtimeErrorCode(err) !== 'ENOENT') errors.push({ path: plan.destination, code: 'destination-stat-error', message: `发布前无法确认目标状态: ${runtimeErrorMessage(err)}` });
      }
    ...
    if (errors.length === 0) {
      try {
        ...
        errors.push({ path: plan.destination, code: 'publish-failed', message: `暂存目录无法放到最终包名: ${runtimeErrorMessage(err)}` });
      }
    }
    if (errors.length > 0) {
      return {
    ...
    for (const file of metadataFiles) {
      try {
        ...
        errors.push({ code: file.mismatchCode, message: '发布后 ' + file.name + ' 不可读取或解析: ' + runtimeErrorMessage(err) });
      }
    }
...
export = { PACKS_DIR_SEGMENTS, PACK_NAME_RE, STAGING_PREFIX, UsageError, BYTE_ONLY_NOTE, validatePackName, checkDestination, loadVerifiedManifest, planResourcePack, applyPackPlan, stageResourcePack };
