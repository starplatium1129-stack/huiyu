...
const { SCHEMA_VERSION, UsageError, compareManifests, resolveRealpathBoundary } = require('./resource-manifest');
const { BYTE_ONLY_NOTE, checkDestination, loadVerifiedManifest, applyPackPlan } = require('./resource-pack');
...
function manifestContentIdentity(manifest) {
  const triples = (manifest && Array.isArray(manifest.entries) ? manifest.entries : [])
    .map((e) => [e.path, e.bytes, String(e.sha256 || '').toLowerCase()])
    .sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0));
  return sha256Hex(Buffer.from(JSON.stringify(triples), 'utf8'));
}

function manifestIdentityRecord(rel, manifest) {
  return {
    path: rel,
 ...
 * 都会阻止发布）。
 */
function loadBaseManifestStructure({ rootReal, manifestPath, io }) {
  ...
  try {
    ...
    throw new UsageError(`--base-manifest 不可读取: ${err.message}`);
  }
  let manifest;
  try {
    manifest = JSON.parse(raw.toString('utf8'));
  } catch (err) {
    return { ok: false, manifest: null, manifestRel: rel, errors: [{ code: 'bad-manifest', message: `旧清单 JSON 解析失败: ${err.message}` }] };
  }
 ...
 */
function planResourcePackDelta({ root, name, manifestPath, baseManifestPath, io = nodeFs } = {}) {
  if (!manifestPath || typeof manifestPath !== 'string') {
  ...
  if (errors.length === 0) {
    ...
    if (!diff.ok) {
      ...
      const newByPath = new Map(loaded.manifest.entries.map((e) => [e.path, e]));
      candidates = [...diff.added, ...diff.changed]
...
function deltaMetadataFiles({ plan, destAbs, rootReal }) {
 ...
 */
function stageResourcePackDelta({ root, name, manifestPath, baseManifestPath, io = nodeFs, platform = process.platform } = {}) {
...
module.exports = { IDENTITY_BASIS, manifestContentIdentity, planResourcePackDelta, stageResourcePackDelta };
