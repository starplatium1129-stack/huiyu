...
const { context, access, locked, child, noLinks, mkdir, readJson, writeJson, unlink, ensureSpace,
  cancelled, event, fail, flushDir, cleanAtomicTemps } = require('./resource-install-fs');
const { HASH, releasePolicy, reference, readPack, targetManifest, verifyTree } = require('./resource-install-policy');
const { equal, readState, verifyVersion, existingVersion, nextState, readJournal } = require('./resource-install-state');
const { copyEntry, fileMatches } = require('./resource-install-copy');
...
function finishInterruptedSwitch(ctx, journal, state) {
  ...
  try {
    ...
    forgetJournal(ctx);
    return result(ctx, journal.before, 'rolled-back', { cause: { code: error.code, message: error.message } });
  }
  forgetJournal(ctx);
  ...
  try {
    ...
    if (equal(current, next)) {
      try {
        ...
        error.rollbackError = { code: rollbackError.code, message: rollbackError.message };
      }
    }
    throw error;
  }
...
async function performInstall(ctx, releaseId, signal) {
  ...
  if (journal) {
    ...
    try { existing = existingVersion(ctx, release.targetIdentity); }
      ...
      if (!['CONTENT_INVALID', 'INSTALLED_TAMPERED', 'UNLISTED_FILE'].includes(error.code)) throw error;
      quarantineFailedTarget(ctx, journal);
    ...
    async status() {
      ...
      return locked(ctx, async () => {
        ...
        if (state.previous) {
          ...
          catch (error) { previous = { ok: false, code: error.code, message: error.message }; }
        }
...
module.exports = { createResourceInstaller };
