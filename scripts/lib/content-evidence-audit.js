...
const { object } = require('./content-history-reader');
const { hash, jsonHash, SHA, evidencePath, evidenceReader, explicitSource } = require('./content-evidence-io');
const { UUID, validateRecord, inputVersion, reviewState, validateDecisions } = require('./content-evidence-contract');

function sourceEvidence(record, reader, options) {
  const files = [];
  for (const [role, declaration, allowed] of [
    ...record.sources.map((source) => ['source', source, options.sources || []]),
    ...
    } catch (error) { item.status = error.code === 'ENOENT' ? 'missing' : 'invalid'; item.reason = error.message; }
  }
...
function assetEvidence(reader, file, declared) {
  try {
  ...
  } catch (error) { return { status: error.code === 'ENOENT' ? 'missing' : 'invalid', file, reason: error.message, imageQuality: 'unverified' }; }
}

function publicationState(record, item, publication, publishedReader, bindings) {
  ...
  let file;
  try { file = evidencePath(receipt.path); } catch (error) { return { status: 'invalid', reason: error.message, activation: 'unknown' }; }
...
function auditContentEvidence(options) {
  ...
  try {
    ...
    const nearby = (file) => path.posix.join(directory, evidencePath(file));
  ...
  } catch (error) { result.structure.status = 'invalid'; result.errors.push(error.message); }
...
module.exports = { auditContentEvidence, sourceEvidence, assetEvidence, publicationState };
