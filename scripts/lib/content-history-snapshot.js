...
const { object, validId, canonicalSceneId } = require('./content-history-reader');
...
const keyFor = (kind, id, characterId = null) => JSON.stringify([kind, characterId, id]);

function loadDomain(reader, domain) {
  const result = { domain, complete: true, rows: [], groups: {}, metadata: {}, unknown: [], issues: [], checks: [], files: [] };
  const issue = (file, reason) => result.issues.push({ file, reason });
  const unknown = (file, reason) => { result.complete = false; result.unknown.push(`${file}: ${reason}`); };
  const json = (file) => {
    if (!result.files.includes(file)) result.files.push(file);
    return reader.json(file);
  };
  const read = (file, validate) => {
    const value = json(file);
    if (!validate(value)) throw new Error(`${file}: invalid container/IDs`);
    return value;
  };
  const validRows = (rows, scene = false) => Array.isArray(rows) && rows.every((row) => object(row) && (scene ? canonicalSceneId(row.id) : validId(row.id)));
  const put = (group, file, kind, id, value, extra = {}) => {
  ...
  const entities = (group, file, values) => {
    result.groups[group] ||= { complete: true, rows: [] };
  ...
  const product = (file, expected, validate, rowSelector = (v) => v) => {
    try {
      ...
      result.checks.push({ file, status: 'unknown', reason: error.message, scope: 'domain-projection' });
      unknown(file, error.message);
    }
  };
  ...
  if (Object.hasOwn(PRODUCTS, domain)) {
    ...
    const shape = domain === 'scenes' ? (v) => validRows(v, true) : (v) => object(v) && validRows(v[recordsKey]);
    let sourceComplete = true;
    const sourceRows = [];
    try {
      const manifestFile = `${directory}/manifest.json`;
      const manifest = read(manifestFile, (v) => object(v) && Array.isArray(v.files)
        && v.files.every((e) => object(e) && typeof e.file === 'string' && /^[^/\\:\x00]+\.json$/.test(e.file)
          && e.file !== 'manifest.json' && e.file !== '..json')
        && new Set(v.files.map((e) => e.file)).size === v.files.length);
      ...
      for (const entry of manifest.files) {
        ...
        try {
          if (domain === 'scenes') {
            ...
            const batches = names.filter((file) => file.startsWith(`${stem}.`) && /^\d+\.json$/.test(file.slice(stem.length + 1)))
              .sort((a, b) => Number(a.slice(stem.length + 1, -5)) - Number(b.slice(stem.length + 1, -5)));
            if (batches.length) {
              files = batches;
              if (names.includes(entry.file) || batches.some((file, i) => file !== `${stem}.${i + 1}.json`)) throw new Error('ambiguous source: batch gap/noncanonical number or single file coexists');
            }
          }
          const loaded = [];
        ...
        } catch (error) { sourceComplete = false; unknown(`${directory}/${entry.file}`, error.message); }
      }
    ...
    } catch (error) { sourceComplete = false; unknown(directory, error.message); }
    ...
    if (domain === 'scenes') {
      ...
      let coreIds;
      try {
        const curation = read('data/curation.json', (v) => object(v) && TIERS.every((tier) => Array.isArray(v[tier]) && v[tier].every(canonicalSceneId)));
        result.metadata['data/curation.json'] = curation;
        coreIds = curation.personaCoreSceneIds.slice(0, 2000).filter((id) => seen.has(id));
      } catch (error) { unknown('data/curation.json', error.message); }
      const byId = new Map(sourceRows.map((row) => [row.id, row]));
      product('data/scenes-core.json', sourceComplete && coreIds ? coreIds.map((id) => byId.get(id)) : undefined, shape);
      ...
      } catch (error) { unknown('data/scenes-index.json', error.message); }
    }
    ...
    } catch (error) { result.groups[group].complete = false; unknown(file, error.message); }
    ...
    for (const [role, file] of [['source', 'data/character-reference-standards.json'], ['derived', 'data/character-reference-view.json']]) {
      ...
      try {
        ...
        if (!validRows(characters) || characters.some((row) => !Array.isArray(row.outfits)
          || row.outfits.some((outfit) => !object(outfit) || !validId(role === 'source' ? outfit.id : outfit.outfitId)
            || (role === 'derived' && (!Array.isArray(outfit.references) || outfit.references.some((ref) => !object(ref) || !validId(ref.id))))))) throw new Error('invalid reference character/outfit records');
        if (role === 'source') {
...
module.exports = { loadDomain, TIERS, PRODUCTS, keyFor };
