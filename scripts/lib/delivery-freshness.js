'use strict';
const { object, canonical, relative, fileEntry } = require('./delivery-paths');
const { HASH, compareSnapshot } = require('./delivery-identity');
const { get, state, gatePath, MAIN_FIELDS } = require('./delivery-state');
const { isReboundGate } = require('./delivery-finalize');
...
function inspectGate(root, document, freshness, field) {
  ...
  } catch (error) { result.status = 'invalid'; result.message = error.message; }
...
module.exports = { inspectTracking, inspectGate, bindGate };
