'use strict';
const { object, canonical, sha256, readJson } = require('./delivery-paths');
const { validateSnapshot } = require('./delivery-identity');
const { get, set, state, MAIN_FIELDS } = require('./delivery-state');
const { COMMIT, ancestor, sourceProof } = require('./delivery-git-proof');
...
function origins(root, nodes, appended) {
  ...
  const { bindGate } = require('./delivery-freshness');
...
module.exports = { createFinalization, inspectFinalization, isReboundGate };
