...
const { SCHEMA_VERSION, UsageError, compareManifests, resolveRealpathBoundary, verifyManifestEntries } = require('./resource-manifest');
...
function identityRecordErrors(record, field, errors) {
...
function checkDeltaMetadataForm(delta) {
  const errors = [];
  ...
  if (!Array.isArray(delta.removed)) {
    ...
    delta.removed.forEach((item, index) => {
...
function verifyDeltaPackContent({ baseManifest, packManifest, delta } = {}) {
  ...
  const baseByPath = new Map(baseEntries.map((e) => [e.path, e]));
  const packByPath = new Map(packEntries.map((e) => [e.path, e]));
...
function isInsideRoot(rootReal, abs) {
...
function resolveInsideRootPath(rootReal, relOrAbs, flag) {
...
function readJsonFile({ absPath, flag, rootReal, io, allowMissing = false }) {
  ...
  try {
    ...
    if (allowMissing && err.code === 'ENOENT') return { missing: true };
...
function resolvePackDir({ rootReal, packPath, io }) {
  ...
  try {
    ...
    throw new UsageError(`--pack 候选目录不可访问: ${err.message}`);
  }
...
module.exports = { DELTA_KIND, VERIFY_SCOPE, verifyDeltaPackContent, verifyDeltaPack, UsageError };
