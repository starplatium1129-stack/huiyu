...
const { object, canonical, sha256, relative, within, excluded, resolveSafe, fileEntry } = require('./delivery-paths');
const HASH = /^[a-f\d]{64}$/;
...
function snapshot(root, input) {
  ...
  function visit(name, kind, selectedRoot = false) {
    ...
    try {
      ...
      entries.push({ path: name, status: error.code === 'ENOENT' ? 'missing' : 'unsafe-or-unreadable', message: error.message });
    }
  }
...
function compareSnapshot(root, recorded) {
  ...
  } catch (error) { return { status: 'invalid', message: error.message, changes: [] }; }
}
function repository(root) {
  ...
  } catch (error) { return { status: 'unknown', commit: null, worktree: 'unknown', message: error.message }; }
}
module.exports = { HASH, selectors, snapshot, validateSnapshot, compareSnapshot, repository };
