...
const { context, locked, access, child, mkdir, noLinks, readJson, writeJson, writeAtomic, unlink,
  ensureSpace, writeAll, event, cancelled, fail, digest, flushDir, ResourceError, cleanAtomicTemps } = require('./resource-install-fs');
const { releasePolicy, decodePack, readPack, verifyTree } = require('./resource-install-policy');
const { fileMatches } = require('./resource-install-copy');
const { sourceUrl, response, fetchMetadata, rangeStart } = require('./resource-download-http');

async function downloadEntry(ctx, release, pack, parts, entry, signal, timeoutMs) {
...
async function publishEntry(ctx, partial, target, entry, signal) {
  cancelled(signal);
...
function createResourceDownloader(options) {
      ...
      return locked(ctx, async () => {
        ...
        if (done?.packageIdentity === release.packageIdentity) {
          try {
            ...
            if (!['CONTENT_INVALID', 'PACKAGE_UNAPPROVED', 'TARGET_MISMATCH', 'METADATA_INVALID'].includes(error.code)) throw error;
...
module.exports = { createResourceDownloader };
