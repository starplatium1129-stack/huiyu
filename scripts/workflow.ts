...
const WORKFLOWS: import('./lib/workflow-types').RegisteredWorkflows = {
  ...
  'desktop:electron-prepare': { desc: '校验并解压官方固定版本 Electron 到独立 PoC runtime，不安装', cmd: ['node', 'scripts/maintenance/prepare-electron-poc.mjs'], required: ['--out'], opts: '--out <新runtime目录>', docs: 'docs/workflow.md',
    run: { nature: ['isolated-fixture', 'network-download', 'writes-product'], machine: ['windows', 'node', 'network'], switches: {}, resume: 'na', evidence: 'scripts/maintenance/prepare-electron-poc.mts', unknown: ['准备runtime不代表壳验收通过'], notes: ['44.4.5官方GitHub校验表、固定SHA256与npm版本互核；同版本本机Electron缓存优先；不改根依赖或安装应用'] } },
  'desktop:electron-check': { desc: '隔离 Electron 三窗、私库、runtime恢复、渲染故障及Live2D真实验收', cmd: ['node', 'scripts/tests/test-electron-shell.mjs'], required: ['--electron', '--node', '--gateway-root', '--ui-root', '--native-exe', '--out'],
    opts: '--electron <verified electron.exe> --node <pinned node.exe> --gateway-root <built gateway> --ui-root <dist> --native-exe <R12候选> --out <新证据目录> [--assets-root <现有assets>] [--sample-ms 2000] [--skip-native] [--skip-metrics] [--resume-native <旧报告>|--resume-costs <旧报告>]', docs: 'docs/workflow.md',
    run: { nature: ['isolated-fixture', 'writes-product', 'service'], machine: ['windows', 'node', 'build-present'], switches: { '--skip-native': ['isolated-fixture'], '--skip-metrics': ['isolated-fixture'] }, resume: 'na', evidence: 'scripts/tests/test-electron-shell.mts', unknown: ['新框架不自动胜出；截图需视觉审查；GPU计数器不可用时记录null'], notes: ['显式新profile/config/AI目录，真实SQLite空迁移后激活；三窗/runtime/renderer故障、旧会话失效、写入重开；不安装、不使用生产profile或调用AI，与Tauri/GPU测量串行；skip-native只作smoke不计模型验收，skip-metrics跳过成本采样'] } },
  'desktop:renderer-process-check': { desc: '使用已有模型在隔离 Windows 进程验证 Live2D 画面、帧率与故障回收', cmd: ['node', 'scripts/tests/test-live2d-renderer-process.mjs'], required: ['--exe', '--assets-root', '--out'],
    opts: '--exe <已构建候选EXE> --assets-root <已有assets目录> --out <新证据目录> [--sample-ms 2000]', docs: 'docs/workflow.md#门禁与构建',
    run: { nature: ['isolated-fixture', 'writes-product'], machine: ['windows', 'node', 'build-present'], switches: {}, resume: 'na', evidence: 'scripts/tests/test-live2d-renderer-process.mts', unknown: ['不同机器的GPU性能；快照需要实际视觉审查'], notes: ['真实本地GPU与可见隔离窗口；需支持renderer child/probe协议的候选；仅读取已有宁宁和夏目资源，不下载、不调用AI生成、不启动安装版或读取生产profile；与其他GPU测量串行；帧率只记录实测，不把目标值当成达成值'] } },
