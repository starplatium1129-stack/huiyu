...
const WORKFLOWS = {
  ...
  'audit:resource-manifest': { desc: '只读本地资源清单生成与校验：root/assets 路径、字节、SHA-256，零写入', cmd: ['node', 'scripts/maintenance/report-resource-manifest.js'],
    opts: '--root <目录>；--manifest <root内JSON>（校验模式：重复/非法/越界路径、缺失、字节与哈希）', docs: 'docs/workflow.md',
    run: { nature: ['read-only'], machine: ['node'], switches: { '--manifest': ['guard'] }, resume: 'na', evidence: 'scripts/maintenance/report-resource-manifest.js:1', unknown: ['哈希一致只证明字节相同；文件存在不代表内容已交付、图片质量或审核通过；校验只核对已列条目，不发现未登记文件'] } },
