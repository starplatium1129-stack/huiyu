...
最新复核：A、RB、RC、N1/N2/N3、G1–G16 已整合；RD 的部分用例仍未验收。记录见 [汇总复核](../../research/engineering/six-task-review-2026-09-13.md)。G14/G15/G16 已结束，不重复启动；下一步由主任务接蓝图保存锁、统一快照与回滚，现有适配器不代表保存路由已修好。用户另有 UI 设计升级会话，后端任务不得修改或暂存其文件，也不使用共享构建输出。查修前版本使用 git show，不得 stash、切分支或自行提交。
