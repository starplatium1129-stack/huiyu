...
> 本版替代 PR #10 初稿 `188d9b6e` 的执行顺序与协议说明。状态：**R0 已完成；R1 仓储收口与最小 bootstrap 实现/定向验收完成；R2–R13 待执行**。实际结果与限制见 [R0 记录](R0-EXECUTION-REPORT.md)、[R1 记录](R1-EXECUTION-REPORT.md)；原生安装/WebView2 未验，尚未激活新存储权威或切换 UI 来源。
...
R0–R11 是主线；R12/R13 为独立实验。R0 已完成，R1 实现与定向验收完成（原生安装/WebView2 未验），R2–R13 仍待执行。每批文件与命令详见执行手册，不能只完成文档就勾选代码交付。
...
方案复审当时未运行项目测试或实施护栏；R0 的实际命令、失败及证据单列于[实施记录](R0-EXECUTION-REPORT.md)。后续 R1 的仓储接线、最小 bootstrap 与当次验收见 [R1 记录](R1-EXECUTION-REPORT.md)，未迁移用户数据或切换来源。Windows 原生、真实数据库和最终性能仍由各批门槛验证，不能用方案写得详细替代实测。
