...
> 本版替代 PR #10 初稿 `188d9b6e` 的执行顺序与协议说明。状态：**R0 已完成；R1 实现/定向验收完成；R2 代码、隔离及 sidecar 门槛完成，默认关闭；R3–R13 待执行**。实际结果与限制见 [R0 记录](R0-EXECUTION-REPORT.md)、[R1 记录](R1-EXECUTION-REPORT.md)、[R2 记录](R2-EXECUTION-REPORT.md)；原生安装/WebView2 未验，尚未启用生产 workspace 或切换 UI 来源。
...
R0–R11 是主线；R12/R13 为独立实验。R0 已完成，R1 实现与定向验收完成，R2 代码、隔离及 sidecar 门槛完成但未启用生产；原生安装/WebView2 未验，R3–R13 仍待执行。每批文件与命令详见执行手册，不能只完成文档就勾选代码交付。
