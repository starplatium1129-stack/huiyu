...
function run(args) { return cp.spawnSync('npx', args, { encoding: 'utf8', timeout: 120000, maxBuffer: 20 * 1024 * 1024 }); }
...
const oldReport = JSON.parse(fs.readFileSync('/tmp/huiyu-refresh-before.json', 'utf8'));
assert.equal(oldReport.numTotalTests, 4);
assert.equal(oldReport.numFailedTests, 3);
...
const fixedReport = JSON.parse(fs.readFileSync('/tmp/huiyu-refresh-after.json', 'utf8'));
const changes = [];
function edit(file, selector, expected, patch) {
    const data = JSON.parse(fs.readFileSync(file, 'utf8')), record = selector(data);
    assert(record, `${file}: target record missing`);
    for (const [field, value] of Object.entries(expected))
        assert.deepEqual(record[field], value, `${file}/${record.id}.${field}: unexpected source drift`);
...
for (const [id, [before, after]] of Object.entries(surtr))
    edit('data/blueprints/arknights.json', (d) => d.blueprints.find(b => b.id === id), { promptProse: before }, { promptProse: after });
...
const marin = JSON.parse(fs.readFileSync(marinFile, 'utf8')).blueprints.find(b => b.id === 'kitagawa_marin_gyaru_street');
assert(marin.negativeTokens.includes('day'));
edit(marinFile, (d) => d.blueprints.find(b => b.id === marin.id), { negativeTokens: marin.negativeTokens }, { negativeTokens: marin.negativeTokens.filter(t => t !== 'day') });
