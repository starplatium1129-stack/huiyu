...
export async function storeAdmittedImage(root: string, buffer: Buffer, prefix: string, extension: string, owner: string, limits: Limits = {}): Promise<string> {
  return queue.run(async () => {
    ...
    try {
      const name = prefix + crypto.createHash('sha256').update(owner).update('\0').update(buffer).digest('hex').slice(0, 40) + '.' + extension;
